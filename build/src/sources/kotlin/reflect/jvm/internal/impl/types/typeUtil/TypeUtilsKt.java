...
public final class TypeUtilsKt {
    ...
    public static final boolean containsTypeAliasParameters(@NotNull KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        return contains(kotlinType, b.f34811d);
    }

    public static final boolean containsTypeParameter(@NotNull KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        return TypeUtils.contains(kotlinType, a.f34810d);
    }
    ...
    public static final boolean f(UnwrappedType it) {
        ...
        ClassifierDescriptor mo1199getDeclarationDescriptor = it.getConstructor().mo1199getDeclarationDescriptor();
        if (mo1199getDeclarationDescriptor != null) {
            return isTypeAliasParameter(mo1199getDeclarationDescriptor);
        }
        return false;
    }
    ...
    public static final KotlinType getRepresentativeUpperBound(@NotNull TypeParameterDescriptor typeParameterDescriptor) {
        ...
        while (true) {
            ...
            Object next = it.next();
            ClassifierDescriptor mo1199getDeclarationDescriptor = ((KotlinType) next).getConstructor().mo1199getDeclarationDescriptor();
    ...
    private static final void h(KotlinType kotlinType, KotlinType kotlinType2, Set set, Set set2) {
        ...
        if (mo1199getDeclarationDescriptor instanceof TypeParameterDescriptor) {
            if (!Intrinsics.areEqual(kotlinType.getConstructor(), kotlinType2.getConstructor())) {
                set.add(mo1199getDeclarationDescriptor);
                return;
            }
            for (KotlinType kotlinType3 : ((TypeParameterDescriptor) mo1199getDeclarationDescriptor).getUpperBounds()) {
                Intrinsics.checkNotNull(kotlinType3);
                h(kotlinType3, kotlinType2, set, set2);
            }
            return;
        }
        ClassifierDescriptor mo1199getDeclarationDescriptor2 = kotlinType.getConstructor().mo1199getDeclarationDescriptor();
        if (mo1199getDeclarationDescriptor2 instanceof ClassifierDescriptorWithTypeParameters) {
            classifierDescriptorWithTypeParameters = (ClassifierDescriptorWithTypeParameters) mo1199getDeclarationDescriptor2;
        } else {
            classifierDescriptorWithTypeParameters = null;
        }
        if (classifierDescriptorWithTypeParameters != null) {
        ...
        for (TypeProjection typeProjection : kotlinType.getArguments()) {
            ...
            if ((typeParameterDescriptor == null || set2 == null || !set2.contains(typeParameterDescriptor)) && !typeProjection.isStarProjection() && !CollectionsKt.d0(set, typeProjection.getType().getConstructor().mo1199getDeclarationDescriptor()) && !Intrinsics.areEqual(typeProjection.getType().getConstructor(), kotlinType2.getConstructor())) {
                KotlinType type = typeProjection.getType();
                Intrinsics.checkNotNullExpressionValue(type, "getType(...)");
    ...
    public static final boolean i(UnwrappedType it) {
        ...
        if (!(mo1199getDeclarationDescriptor instanceof TypeAliasDescriptor) && !(mo1199getDeclarationDescriptor instanceof TypeParameterDescriptor)) {
            return false;
        }
        return true;
    }
    ...
    public static final KotlinType replaceArgumentsWithStarProjections(@NotNull KotlinType kotlinType) {
        ...
        if (unwrap instanceof FlexibleType) {
            ...
            if (!isEmpty) {
                ClassifierDescriptor mo1199getDeclarationDescriptor = simpleType2.getConstructor().mo1199getDeclarationDescriptor();
                simpleType = simpleType2;
                if (mo1199getDeclarationDescriptor != null) {
    ...
    public static final boolean requiresTypeAliasExpansion(@NotNull KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        return contains(kotlinType, c.f34812d);
    }

    public static final boolean shouldBeUpdated(KotlinType kotlinType) {
        if (kotlinType != null && !contains(kotlinType, d.f34813d)) {
            return false;
        }
        return true;
    }
