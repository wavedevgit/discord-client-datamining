package com.facebook.react.viewmanagers;

import android.view.View;
import com.discord.misc.utilities.chat_view_types.ChatViewRecyclerTypes;
import com.facebook.react.bridge.ColorPropConverter;
import com.facebook.react.bridge.DynamicFromObject;
import com.facebook.react.bridge.ReadableArray;
import com.facebook.react.devsupport.StackTraceHelper;
import com.facebook.react.uimanager.BaseViewManager;
import com.facebook.react.uimanager.BaseViewManagerDelegate;
import com.facebook.react.uimanager.LayoutShadowNode;
import com.facebook.react.uimanager.ViewProps;
import com.facebook.react.viewmanagers.RNSVGPatternManagerInterface;
/* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes3.dex */
public class RNSVGPatternManagerDelegate<T extends View, U extends BaseViewManager<T, ? extends LayoutShadowNode> & RNSVGPatternManagerInterface<T>> extends BaseViewManagerDelegate<T, U> {
    /* JADX WARN: Incorrect types in method signature: (TU;)V */
    public RNSVGPatternManagerDelegate(BaseViewManager baseViewManager) {
        super(baseViewManager);
    }

    @Override // com.facebook.react.uimanager.BaseViewManagerDelegate, com.facebook.react.uimanager.ViewManagerDelegate
    public void setProperty(T t10, String str, Object obj) {
        str.getClass();
        int i10 = 1;
        int i11 = 0;
        int i12 = 0;
        boolean z10 = false;
        int i13 = 0;
        int i14 = 0;
        int i15 = 0;
        int i16 = 0;
        int i17 = 0;
        char c10 = 65535;
        switch (str.hashCode()) {
            case -1567958285:
                if (str.equals("vbHeight")) {
                    c10 = 0;
                    break;
                }
                break;
            case -1274492040:
                if (str.equals(ViewProps.FILTER)) {
                    c10 = 1;
                    break;
                }
                break;
            case -1267206133:
                if (str.equals(ViewProps.OPACITY)) {
                    c10 = 2;
                    break;
                }
                break;
            case -1221029593:
                if (str.equals("height")) {
                    c10 = 3;
                    break;
                }
                break;
            case -1081239615:
                if (str.equals("matrix")) {
                    c10 = 4;
                    break;
                }
                break;
            case -993894751:
                if (str.equals("propList")) {
                    c10 = 5;
                    break;
                }
                break;
            case -933864895:
                if (str.equals("markerEnd")) {
                    c10 = 6;
                    break;
                }
                break;
            case -933857362:
                if (str.equals("markerMid")) {
                    c10 = 7;
                    break;
                }
                break;
            case -891980232:
                if (str.equals("stroke")) {
                    c10 = '\b';
                    break;
                }
                break;
            case -734428249:
                if (str.equals(ViewProps.FONT_WEIGHT)) {
                    c10 = '\t';
                    break;
                }
                break;
            case -729118945:
                if (str.equals("fillRule")) {
                    c10 = '\n';
                    break;
                }
                break;
            case -416535885:
                if (str.equals("strokeOpacity")) {
                    c10 = 11;
                    break;
                }
                break;
            case -293492298:
                if (str.equals(ViewProps.POINTER_EVENTS)) {
                    c10 = '\f';
                    break;
                }
                break;
            case -207800897:
                if (str.equals("patternUnits")) {
                    c10 = '\r';
                    break;
                }
                break;
            case -128680410:
                if (str.equals("patternContentUnits")) {
                    c10 = 14;
                    break;
                }
                break;
            case -53677816:
                if (str.equals("fillOpacity")) {
                    c10 = 15;
                    break;
                }
                break;
            case -44578051:
                if (str.equals("strokeDashoffset")) {
                    c10 = 16;
                    break;
                }
                break;
            case 120:
                if (str.equals("x")) {
                    c10 = 17;
                    break;
                }
                break;
            case 121:
                if (str.equals("y")) {
                    c10 = 18;
                    break;
                }
                break;
            case 3143043:
                if (str.equals("fill")) {
                    c10 = 19;
                    break;
                }
                break;
            case 3148879:
                if (str.equals("font")) {
                    c10 = 20;
                    break;
                }
                break;
            case 3344108:
                if (str.equals("mask")) {
                    c10 = 21;
                    break;
                }
                break;
            case 3351622:
                if (str.equals("minX")) {
                    c10 = 22;
                    break;
                }
                break;
            case 3351623:
                if (str.equals("minY")) {
                    c10 = 23;
                    break;
                }
                break;
            case 3373707:
                if (str.equals(StackTraceHelper.NAME_KEY)) {
                    c10 = 24;
                    break;
                }
                break;
            case 78845486:
                if (str.equals("strokeMiterlimit")) {
                    c10 = 25;
                    break;
                }
                break;
            case 92903173:
                if (str.equals("align")) {
                    c10 = 26;
                    break;
                }
                break;
            case 94842723:
                if (str.equals(ViewProps.COLOR)) {
                    c10 = 27;
                    break;
                }
                break;
            case 104482996:
                if (str.equals("vectorEffect")) {
                    c10 = 28;
                    break;
                }
                break;
            case 113126854:
                if (str.equals("width")) {
                    c10 = 29;
                    break;
                }
                break;
            case 217109576:
                if (str.equals("markerStart")) {
                    c10 = 30;
                    break;
                }
                break;
            case 240482938:
                if (str.equals("vbWidth")) {
                    c10 = 31;
                    break;
                }
                break;
            case 365601008:
                if (str.equals(ViewProps.FONT_SIZE)) {
                    c10 = ' ';
                    break;
                }
                break;
            case 401643183:
                if (str.equals("strokeDasharray")) {
                    c10 = '!';
                    break;
                }
                break;
            case 746561980:
                if (str.equals("patternTransform")) {
                    c10 = '\"';
                    break;
                }
                break;
            case 917656469:
                if (str.equals("clipPath")) {
                    c10 = '#';
                    break;
                }
                break;
            case 917735020:
                if (str.equals("clipRule")) {
                    c10 = '$';
                    break;
                }
                break;
            case 1027575302:
                if (str.equals("strokeLinecap")) {
                    c10 = '%';
                    break;
                }
                break;
            case 1671764162:
                if (str.equals(ViewProps.DISPLAY)) {
                    c10 = '&';
                    break;
                }
                break;
            case 1790285174:
                if (str.equals("strokeLinejoin")) {
                    c10 = '\'';
                    break;
                }
                break;
            case 1847674614:
                if (str.equals("responsible")) {
                    c10 = '(';
                    break;
                }
                break;
            case 1908075304:
                if (str.equals("meetOrSlice")) {
                    c10 = ')';
                    break;
                }
                break;
            case 1924065902:
                if (str.equals("strokeWidth")) {
                    c10 = '*';
                    break;
                }
                break;
        }
        float f10 = 1.0f;
        float f11 = 0.0f;
        String str2 = null;
        switch (c10) {
            case 0:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface.setVbHeight(t10, f11);
                return;
            case 1:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface2 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface2.setFilter(t10, str2);
                return;
            case 2:
                U u10 = this.mViewManager;
                if (obj != null) {
                    f10 = ((Double) obj).floatValue();
                }
                u10.setOpacity(t10, f10);
                return;
            case 3:
                ((RNSVGPatternManagerInterface) this.mViewManager).setHeight(t10, new DynamicFromObject(obj));
                return;
            case 4:
                ((RNSVGPatternManagerInterface) this.mViewManager).setMatrix(t10, (ReadableArray) obj);
                return;
            case 5:
                ((RNSVGPatternManagerInterface) this.mViewManager).setPropList(t10, (ReadableArray) obj);
                return;
            case 6:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface3 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface3.setMarkerEnd(t10, str2);
                return;
            case 7:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface4 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface4.setMarkerMid(t10, str2);
                return;
            case '\b':
                ((RNSVGPatternManagerInterface) this.mViewManager).setStroke(t10, new DynamicFromObject(obj));
                return;
            case '\t':
                ((RNSVGPatternManagerInterface) this.mViewManager).setFontWeight(t10, new DynamicFromObject(obj));
                return;
            case '\n':
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface5 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i10 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface5.setFillRule(t10, i10);
                return;
            case 11:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface6 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f10 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface6.setStrokeOpacity(t10, f10);
                return;
            case '\f':
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface7 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface7.setPointerEvents(t10, str2);
                return;
            case '\r':
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface8 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i11 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface8.setPatternUnits(t10, i11);
                return;
            case 14:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface9 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i17 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface9.setPatternContentUnits(t10, i17);
                return;
            case 15:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface10 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f10 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface10.setFillOpacity(t10, f10);
                return;
            case 16:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface11 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface11.setStrokeDashoffset(t10, f11);
                return;
            case 17:
                ((RNSVGPatternManagerInterface) this.mViewManager).setX(t10, new DynamicFromObject(obj));
                return;
            case ChatViewRecyclerTypes.EPHEMERAL_INDICATION /* 18 */:
                ((RNSVGPatternManagerInterface) this.mViewManager).setY(t10, new DynamicFromObject(obj));
                return;
            case ChatViewRecyclerTypes.INTERACTION_STATUS /* 19 */:
                ((RNSVGPatternManagerInterface) this.mViewManager).setFill(t10, new DynamicFromObject(obj));
                return;
            case 20:
                ((RNSVGPatternManagerInterface) this.mViewManager).setFont(t10, new DynamicFromObject(obj));
                return;
            case ChatViewRecyclerTypes.FLAGGED_MESSAGE_EMBED /* 21 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface12 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface12.setMask(t10, str2);
                return;
            case ChatViewRecyclerTypes.FLAGGED_MESSAGE_ACTION_BAR /* 22 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface13 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface13.setMinX(t10, f11);
                return;
            case 23:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface14 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface14.setMinY(t10, f11);
                return;
            case ChatViewRecyclerTypes.SURVEY_INDICATION /* 24 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface15 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface15.setName(t10, str2);
                return;
            case ChatViewRecyclerTypes.GUILD_INVITE_DISABLED /* 25 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface16 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface16.setStrokeMiterlimit(t10, f11);
                return;
            case ChatViewRecyclerTypes.MEDIA_MOSAIC_ATTACHMENT /* 26 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface17 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface17.setAlign(t10, str2);
                return;
            case 27:
                ((RNSVGPatternManagerInterface) this.mViewManager).setColor(t10, ColorPropConverter.getColor(obj, t10.getContext()));
                return;
            case 28:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface18 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i16 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface18.setVectorEffect(t10, i16);
                return;
            case 29:
                ((RNSVGPatternManagerInterface) this.mViewManager).setWidth(t10, new DynamicFromObject(obj));
                return;
            case 30:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface19 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface19.setMarkerStart(t10, str2);
                return;
            case ChatViewRecyclerTypes.EMBEDDED_ACTIVITY_INVITE /* 31 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface20 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    f11 = ((Double) obj).floatValue();
                }
                rNSVGPatternManagerInterface20.setVbWidth(t10, f11);
                return;
            case ' ':
                ((RNSVGPatternManagerInterface) this.mViewManager).setFontSize(t10, new DynamicFromObject(obj));
                return;
            case ChatViewRecyclerTypes.AUTO_MODERATION_NOTIFICATION_EMBED /* 33 */:
                ((RNSVGPatternManagerInterface) this.mViewManager).setStrokeDasharray(t10, new DynamicFromObject(obj));
                return;
            case ChatViewRecyclerTypes.CHANNEL_DEADCHAT_PROMPT_ACTIONS /* 34 */:
                ((RNSVGPatternManagerInterface) this.mViewManager).setPatternTransform(t10, (ReadableArray) obj);
                return;
            case ChatViewRecyclerTypes.INFO_LINK /* 35 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface21 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface21.setClipPath(t10, str2);
                return;
            case ChatViewRecyclerTypes.SAFETY_POLICY_NOTICE /* 36 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface22 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i15 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface22.setClipRule(t10, i15);
                return;
            case ChatViewRecyclerTypes.POLL_TEXT_AND_IMAGE /* 37 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface23 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i14 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface23.setStrokeLinecap(t10, i14);
                return;
            case ChatViewRecyclerTypes.POLL_IMAGE_ONLY /* 38 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface24 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    str2 = (String) obj;
                }
                rNSVGPatternManagerInterface24.setDisplay(t10, str2);
                return;
            case ChatViewRecyclerTypes.SAFETY_SYSTEM_NOTIFICATION /* 39 */:
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface25 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i13 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface25.setStrokeLinejoin(t10, i13);
                return;
            case '(':
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface26 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    z10 = ((Boolean) obj).booleanValue();
                }
                rNSVGPatternManagerInterface26.setResponsible(t10, z10);
                return;
            case ')':
                RNSVGPatternManagerInterface rNSVGPatternManagerInterface27 = (RNSVGPatternManagerInterface) this.mViewManager;
                if (obj != null) {
                    i12 = ((Double) obj).intValue();
                }
                rNSVGPatternManagerInterface27.setMeetOrSlice(t10, i12);
                return;
            case '*':
                ((RNSVGPatternManagerInterface) this.mViewManager).setStrokeWidth(t10, new DynamicFromObject(obj));
                return;
            default:
                super.kotlinCompat$setProperty(t10, str, obj);
                return;
        }
    }
}
