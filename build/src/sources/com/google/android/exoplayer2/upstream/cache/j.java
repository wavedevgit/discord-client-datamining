...
final class j extends ne.c {
    ...
    private static final Pattern f14096r = Pattern.compile("^(.+)\\.(\\d+)\\.(\\d+)\\.v1\\.exo$", 32);

    /* renamed from: s  reason: collision with root package name */
    private static final Pattern f14097s = Pattern.compile("^(.+)\\.(\\d+)\\.(\\d+)\\.v2\\.exo$", 32);

    /* renamed from: t  reason: collision with root package name */
    private static final Pattern f14098t = Pattern.compile("^(\\d+)\\.(\\d+)\\.(\\d+)\\.v3\\.exo$", 32);
    ...
    public static j g(File file, long j10, long j11, h hVar) {
        ...
        File file2 = file;
        Matcher matcher = f14098t.matcher(name);
    ...
    private static File m(File file, h hVar) {
        String str;
        String name = file.getName();
        Matcher matcher = f14097s.matcher(name);
        if (matcher.matches()) {
            str = w0.o1((String) oe.a.e(matcher.group(1)));
        } else {
            matcher = f14096r.matcher(name);
            if (matcher.matches()) {
