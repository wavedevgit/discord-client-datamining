package com.appsflyer.internal;

import android.content.Context;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.NotNull;
/* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes.dex */
public interface AFg1sSDK {

    /* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes.dex */
    public static final class AFa1uSDK {
        final String AFAdRevenueData;
        final float getRevenue;

        public AFa1uSDK(float f10, String str) {
            this.getRevenue = f10;
            this.AFAdRevenueData = str;
        }

        public final boolean equals(Object obj) {
            if (this == obj) {
                return true;
            }
            if (!(obj instanceof AFa1uSDK)) {
                return false;
            }
            AFa1uSDK aFa1uSDK = (AFa1uSDK) obj;
            if (Float.compare(this.getRevenue, aFa1uSDK.getRevenue) == 0 && Intrinsics.areEqual(this.AFAdRevenueData, aFa1uSDK.AFAdRevenueData)) {
                return true;
            }
            return false;
        }

        public final int hashCode() {
            int hashCode;
            int hashCode2 = Float.hashCode(this.getRevenue) * 31;
            String str = this.AFAdRevenueData;
            if (str == null) {
                hashCode = 0;
            } else {
                hashCode = str.hashCode();
            }
            return hashCode2 + hashCode;
        }

        @NotNull
        public final String toString() {
            float f10 = this.getRevenue;
            String str = this.AFAdRevenueData;
            return "BatteryData(level=" + f10 + ", charging=" + str + ")";
        }
    }

    @NotNull
    AFa1uSDK AFAdRevenueData(@NotNull Context context);
}
