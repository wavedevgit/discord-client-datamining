...
public final class PollAnswer {
    ...
    public static final /* synthetic */ void write$Self$chat_release(PollAnswer pollAnswer, CompositeEncoder compositeEncoder, SerialDescriptor serialDescriptor) {
        compositeEncoder.y(serialDescriptor, 0, pollAnswer.answerId);
        compositeEncoder.D(serialDescriptor, 1, PollMedia$$serializer.INSTANCE, pollAnswer.pollMedia);
        compositeEncoder.x(serialDescriptor, 2, pollAnswer.didSelfVote);
        compositeEncoder.y(serialDescriptor, 3, pollAnswer.votes);
        compositeEncoder.w(serialDescriptor, 4, pollAnswer.votesPercentage);
        if (compositeEncoder.z(serialDescriptor, 5) || pollAnswer.isSelected != null) {
            compositeEncoder.s(serialDescriptor, 5, h.f20942a, pollAnswer.isSelected);
        }
        if (compositeEncoder.z(serialDescriptor, 6) || pollAnswer.isVictor != null) {
            compositeEncoder.s(serialDescriptor, 6, h.f20942a, pollAnswer.isVictor);
        }
        if (compositeEncoder.z(serialDescriptor, 7) || pollAnswer.shouldAnimateTransition != null) {
            compositeEncoder.s(serialDescriptor, 7, h.f20942a, pollAnswer.shouldAnimateTransition);
        }
        if (compositeEncoder.z(serialDescriptor, 8) || !Intrinsics.areEqual(pollAnswer.style, "normal")) {
            compositeEncoder.y(serialDescriptor, 8, pollAnswer.style);
        }
    }
