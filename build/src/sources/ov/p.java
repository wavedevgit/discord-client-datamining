package ov;

import com.discord.misc.utilities.chat_view_types.ChatViewRecyclerTypes;
/* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes5.dex */
public class p extends tv.a {

    /* renamed from: a  reason: collision with root package name */
    private final rv.p f42842a;

    /* renamed from: b  reason: collision with root package name */
    private boolean f42843b;

    /* renamed from: c  reason: collision with root package name */
    private int f42844c;

    /* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes5.dex */
    public static class a extends tv.b {
        @Override // tv.e
        public tv.f a(tv.h hVar, tv.g gVar) {
            boolean z10;
            tv.d b10 = gVar.b();
            if (hVar.a() >= qv.d.f47318a) {
                return tv.f.c();
            }
            int c10 = hVar.c();
            int column = hVar.getColumn() + hVar.a();
            if (gVar.a() != null) {
                z10 = true;
            } else {
                z10 = false;
            }
            b n10 = p.n(hVar.getLine(), c10, column, z10);
            if (n10 == null) {
                return tv.f.c();
            }
            int i10 = n10.f42846b;
            q qVar = new q(i10 - hVar.getColumn());
            if ((b10 instanceof p) && p.m((rv.p) b10.f(), n10.f42845a)) {
                return tv.f.d(qVar).a(i10);
            }
            p pVar = new p(n10.f42845a);
            n10.f42845a.o(true);
            return tv.f.d(pVar, qVar).a(i10);
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes5.dex */
    public static class b {

        /* renamed from: a  reason: collision with root package name */
        final rv.p f42845a;

        /* renamed from: b  reason: collision with root package name */
        final int f42846b;

        b(rv.p pVar, int i10) {
            this.f42845a = pVar;
            this.f42846b = i10;
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes5.dex */
    public static class c {

        /* renamed from: a  reason: collision with root package name */
        final rv.p f42847a;

        /* renamed from: b  reason: collision with root package name */
        final int f42848b;

        c(rv.p pVar, int i10) {
            this.f42847a = pVar;
            this.f42848b = i10;
        }
    }

    public p(rv.p pVar) {
        this.f42842a = pVar;
    }

    private static boolean k(Object obj, Object obj2) {
        if (obj == null) {
            if (obj2 == null) {
                return true;
            }
            return false;
        }
        return obj.equals(obj2);
    }

    private static boolean l(CharSequence charSequence, int i10) {
        char charAt;
        if (i10 >= charSequence.length() || (charAt = charSequence.charAt(i10)) == '\t' || charAt == ' ') {
            return true;
        }
        return false;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static boolean m(rv.p pVar, rv.p pVar2) {
        if ((pVar instanceof rv.c) && (pVar2 instanceof rv.c)) {
            return k(Character.valueOf(((rv.c) pVar).p()), Character.valueOf(((rv.c) pVar2).p()));
        }
        if ((pVar instanceof rv.s) && (pVar2 instanceof rv.s)) {
            return k(Character.valueOf(((rv.s) pVar).p()), Character.valueOf(((rv.s) pVar2).p()));
        }
        return false;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static b n(CharSequence charSequence, int i10, int i11, boolean z10) {
        boolean z11;
        c o10 = o(charSequence, i10);
        if (o10 == null) {
            return null;
        }
        rv.p pVar = o10.f42847a;
        int i12 = o10.f42848b;
        int i13 = i11 + (i12 - i10);
        int length = charSequence.length();
        int i14 = i13;
        while (true) {
            if (i12 < length) {
                char charAt = charSequence.charAt(i12);
                if (charAt == '\t') {
                    i14 += qv.d.a(i14);
                } else if (charAt == ' ') {
                    i14++;
                } else {
                    z11 = true;
                    break;
                }
                i12++;
            } else {
                z11 = false;
                break;
            }
        }
        if (z10 && (((pVar instanceof rv.s) && ((rv.s) pVar).q() != 1) || !z11)) {
            return null;
        }
        if (!z11 || i14 - i13 > qv.d.f47318a) {
            i14 = i13 + 1;
        }
        return new b(pVar, i14);
    }

    private static c o(CharSequence charSequence, int i10) {
        char charAt = charSequence.charAt(i10);
        if (charAt != '*' && charAt != '+' && charAt != '-') {
            return p(charSequence, i10);
        }
        int i11 = i10 + 1;
        if (l(charSequence, i11)) {
            rv.c cVar = new rv.c();
            cVar.q(charAt);
            return new c(cVar, i11);
        }
        return null;
    }

    private static c p(CharSequence charSequence, int i10) {
        int length = charSequence.length();
        int i11 = 0;
        for (int i12 = i10; i12 < length; i12++) {
            char charAt = charSequence.charAt(i12);
            if (charAt != ')' && charAt != '.') {
                switch (charAt) {
                    case '0':
                    case ChatViewRecyclerTypes.MEDIA_ATTACHMENT_MOSAIC_IMAGE /* 49 */:
                    case '2':
                    case ChatViewRecyclerTypes.MEDIA_ATTACHMENT_MOSAIC_VISUAL_PLACEHOLDER /* 51 */:
                    case ChatViewRecyclerTypes.REFERRAL /* 52 */:
                    case ChatViewRecyclerTypes.GUILD_PROFILE_INVITE /* 53 */:
                    case ChatViewRecyclerTypes.APP_MESSAGE_EMBED /* 54 */:
                    case ChatViewRecyclerTypes.ACTIVITY_RICH_PRESENCE_INVITE_EMBED /* 55 */:
                    case ChatViewRecyclerTypes.SHARED_CUSTOM_THEME_EMBED /* 56 */:
                    case ChatViewRecyclerTypes.PREMIUM_GROUP_INVITE /* 57 */:
                        i11++;
                        if (i11 > 9) {
                            return null;
                        }
                    default:
                        return null;
                }
            } else {
                if (i11 >= 1) {
                    int i13 = i12 + 1;
                    if (l(charSequence, i13)) {
                        String charSequence2 = charSequence.subSequence(i10, i12).toString();
                        rv.s sVar = new rv.s();
                        sVar.s(Integer.parseInt(charSequence2));
                        sVar.r(charAt);
                        return new c(sVar, i13);
                    }
                }
                return null;
            }
        }
        return null;
    }

    @Override // tv.a, tv.d
    public boolean a() {
        return true;
    }

    @Override // tv.d
    public tv.c b(tv.h hVar) {
        if (hVar.b()) {
            this.f42843b = true;
            this.f42844c = 0;
        } else if (this.f42843b) {
            this.f42844c++;
        }
        return tv.c.b(hVar.getIndex());
    }

    @Override // tv.a, tv.d
    public boolean c(rv.a aVar) {
        if (!(aVar instanceof rv.q)) {
            return false;
        }
        if (this.f42843b && this.f42844c == 1) {
            this.f42842a.o(false);
            this.f42843b = false;
        }
        return true;
    }

    @Override // tv.d
    public rv.a f() {
        return this.f42842a;
    }
}
