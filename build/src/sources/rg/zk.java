package rg;

import android.os.Parcel;
import android.os.Parcelable;
/* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes3.dex */
public final class zk extends jf.a {
    public static final Parcelable.Creator<zk> CREATOR = new al();

    /* renamed from: d  reason: collision with root package name */
    private final String f48939d;

    /* renamed from: e  reason: collision with root package name */
    private final String f48940e;

    /* renamed from: i  reason: collision with root package name */
    private final String f48941i;

    /* renamed from: o  reason: collision with root package name */
    private final boolean f48942o;

    /* renamed from: p  reason: collision with root package name */
    private final int f48943p;

    /* renamed from: q  reason: collision with root package name */
    private final String f48944q;

    /* renamed from: r  reason: collision with root package name */
    private final boolean f48945r;

    public zk(String str, String str2, String str3, boolean z10, int i10, String str4, boolean z11) {
        this.f48939d = str;
        this.f48940e = str2;
        this.f48941i = str3;
        this.f48944q = str4;
        this.f48943p = i10;
        this.f48942o = z10;
        this.f48945r = z11;
    }

    @Override // android.os.Parcelable
    public final void writeToParcel(Parcel parcel, int i10) {
        String str = this.f48939d;
        int a10 = jf.c.a(parcel);
        jf.c.s(parcel, 1, str, false);
        jf.c.s(parcel, 2, this.f48940e, false);
        jf.c.s(parcel, 3, this.f48941i, false);
        jf.c.c(parcel, 4, this.f48942o);
        jf.c.l(parcel, 5, this.f48943p);
        jf.c.s(parcel, 6, this.f48944q, false);
        jf.c.c(parcel, 7, this.f48945r);
        jf.c.b(parcel, a10);
    }
}
