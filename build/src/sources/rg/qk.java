package rg;

import android.graphics.Rect;
import android.os.Parcel;
import android.os.Parcelable;
import java.util.List;
/* loaded from: /home/runner/work/discord-client-datamining/discord-client-datamining/build/classes3.dex */
public final class qk extends jf.a {
    public static final Parcelable.Creator<qk> CREATOR = new sk();

    /* renamed from: d  reason: collision with root package name */
    private final String f47095d;

    /* renamed from: e  reason: collision with root package name */
    private final Rect f47096e;

    /* renamed from: i  reason: collision with root package name */
    private final List f47097i;

    /* renamed from: o  reason: collision with root package name */
    private final String f47098o;

    /* renamed from: p  reason: collision with root package name */
    private final List f47099p;

    public qk(String str, Rect rect, List list, String str2, List list2) {
        this.f47095d = str;
        this.f47096e = rect;
        this.f47097i = list;
        this.f47098o = str2;
        this.f47099p = list2;
    }

    public final Rect b() {
        return this.f47096e;
    }

    public final String c() {
        return this.f47098o;
    }

    public final String d() {
        return this.f47095d;
    }

    public final List e() {
        return this.f47097i;
    }

    public final List f() {
        return this.f47099p;
    }

    @Override // android.os.Parcelable
    public final void writeToParcel(Parcel parcel, int i10) {
        String str = this.f47095d;
        int a10 = jf.c.a(parcel);
        jf.c.s(parcel, 1, str, false);
        jf.c.q(parcel, 2, this.f47096e, i10, false);
        jf.c.w(parcel, 3, this.f47097i, false);
        jf.c.s(parcel, 4, this.f47098o, false);
        jf.c.w(parcel, 5, this.f47099p, false);
        jf.c.b(parcel, a10);
    }
}
