/** chunk id: 959278 params = (module,exports,require) **/
n.d(t, {
    A: () => to
});
var i = n(627968),
    a = n(64700),
    r = n(503698),
    l = n.n(r),
    s = n(492462),
    o = n(873263),
    d = n(110259),
    c = n(156828),
    u = n(607399),
    A = n(311907),
    _ = n(506774),
    h = n(397927),
    m = n(686956),
    g = n(843472),
    p = n(816015),
    E = n(183994),
    I = n(153244),
    f = n(772614),
    C = n(18422),
    T = n(761929),
    N = n(458518),
    S = n(107361),
    x = n(389405),
    v = n(549711),
    b = n(313961),
    y = n(264140),
    O = n(72800),
    L = n(30747),
    R = n(811736),
    P = n(696086),
    D = n(500831),
    j = n(920814),
    M = n(475419),
    w = n(287344),
    U = n(969043),
    G = n(91242),
    k = n(478071),
    V = n(351466),
    B = n(184453),
    H = n(386467),
    F = n(490358),
    Y = n(961973),
    W = n(140354),
    K = n(440293),
    q = n(551036),
    z = n(46533),
    $ = n(467627),
    Q = n(379610),
    X = n(920113),
    Z = n(164956),
    J = n(209700),
    ee = n(9588),
    et = n(939846),
    en = n(862658),
    ei = n(813160),
    ea = n(916768),
    er = n(851109),
    el = n(667429),
    es = n(480680),
    eo = n(767060),
    ed = n(18635),
    ec = n(488803),
    eu = n(969117),
    eA = n(176031),
    e_ = n(710969),
    eh = n(570763),
    em = n(376708),
    eg = n(272720),
    ep = n(366811),
    eE = n(77509),
    eI = n(463347),
    ef = n(334465),
    eC = n(976860),
    eT = n(97469),
    eN = n(718446),
    eS = n(734057),
    ex = n(650048),
    ev = n(696451),
    eb = n(71393),
    ey = n(309010),
    eO = n(519057),
    eL = n(954571),
    eR = n(403362),
    eP = n(661191),
    eD = n(349320),
    ej = n(652215),
    eM = n(746080),
    ew = n(758836),
    eU = n(539916),
    eG = n(613057),
    ek = n(355097),
    eV = n(325593);
let eB = (0, c.Fe)({
        createPromise: () => Promise.resolve().then(n.bind(n, 500831)),
        webpackId: 500831
    }),
    eH = () => (0, i.jsx)("div", {
        className: eV.wG,
        children: (0, i.jsx)(h.y$y, {})
    }),
    eF = (0, c.Fe)({
        createPromise: () => n.e("78870").then(n.bind(n, 256467)),
        webpackId: 256467,
        name: "GuildShopPage",
        renderLoader: eH
    }),
    eY = (0, c.Fe)({
        createPromise: () => n.e("14078").then(n.bind(n, 854721)),
        webpackId: 854721,
        name: "SocialLayerStorefront",
        renderLoader: eH
    }),
    eW = (0, c.Fe)({
        createPromise: () => n.e("8497").then(n.bind(n, 254316)),
        webpackId: 254316,
        name: "ServerMonetizationOnboardingForwarder",
        renderLoader: eH
    }),
    eK = (0, c.Fe)({
        createPromise: () => n.e("33957").then(n.bind(n, 702035)),
        webpackId: 702035,
        name: "GuildHomePage",
        renderLoader: eH
    }),
    eq = (0, c.Fe)({
        createPromise: () => Promise.all([n.e("35894"), n.e("99583")]).then(n.bind(n, 756154)),
        webpackId: 756154,
        name: "MemberSafetyPage",
        renderLoader: eH
    }),
    ez = (0, c.Fe)({
        createPromise: () => Promise.all([n.e("56026"), n.e("16301"), n.e("22191"), n.e("25961"), n.e("50796"), n.e("16939"), n.e("72165")]).then(n.bind(n, 963878)),
        webpackId: 963878,
        name: "ChannelsAndRolesPage",
        renderLoader: eH
    }),
    e$ = (0, c.Fe)({
        createPromise: () => Promise.all([n.e("56026"), n.e("16301"), n.e("22191"), n.e("25961"), n.e("16939"), n.e("39579")]).then(n.bind(n, 57858)),
        webpackId: 57858,
        name: "GuildOnboardingPage",
        renderLoader: eH
    }),
    eQ = (0, c.Fe)({
        createPromise: () => Promise.all([n.e("63379"), n.e("88615")]).then(n.bind(n, 126076)),
        webpackId: 126076,
        name: "GuildPowerupsPage",
        renderLoader: eH
    }),
    eX = (0, c.Fe)({
        createPromise: () => Promise.all([n.e("91284"), n.e("19750")]).then(n.bind(n, 692150)),
        webpackId: 692150,
        name: "GameServerPage",
        renderLoader: eH
    }),
    eZ = e => {
        let {
            match: t
        } = e, n = (0, N.o)(), r = (0, A.bG)([eS.A, ey.A], () => {
            let e = ey.A.getChannelId();
            return eS.A.getChannel(e)
        }), {
            guildId: l,
            channelId: s,
            messageId: d,
            threadId: c
        } = t.params, u = s ?? void 0;
        null == u && null != t.params.gameShopPageIndex && (u = eM.VV.GAME_SHOP);
        let _ = (0, A.bG)([eb.A], () => eb.A.getGuild(l)),
            h = (0, A.bG)([ev.Ay], () => null == l ? null : ev.Ay.getSelfMember(l)),
            g = (0, ec.C$)(l, "ChannelRenderer"),
            E = (0, eu.N)("ChannelRenderer"),
            I = _?.features.has(ej.GuildFeatures.GAME_SERVERS) ?? !1,
            f = (0, A.bG)([Z.A], () => {
                if (null == l) return;
                let e = Z.A.getData(l);
                if (e?.type === J._.SERVER_SHOP) switch (e.initialTab) {
                    case "role_subscriptions":
                        return j.B.GUILD_ROLE_SUBSCRIPTIONS;
                    case "guild_products":
                        return j.B.GUILD_PRODUCTS;
                    default:
                        return
                }
            }),
            C = (0, A.bG)([U.A], () => (0, em.C$)(u) && (0, em.C$)(c), [u, c]);
        a.useEffect(() => {
            C || (null != _ && (0, eC.pX)(ej.BVt.CHANNEL(_.id, (0, eg.A)(_))), (0, ee.E)())
        }, [C, _]);
        let T = null != _,
            S = a.useRef(null);
        a.useEffect(() => {
            T || null == l || S.current === l || (S.current = l, m.A.joinGuild(l, {
                lurker: !0,
                autoNavigate: !1
            }).catch(() => {}))
        }, [T, l]);
        let x = (0, K.$)(l);
        if (null != l && null != u && (0, eM.jq)(u)) switch (u) {
            case eM.VV.ROLE_SUBSCRIPTIONS:
                return x ? (0, i.jsx)(eF, {
                    guildId: l,
                    initialTab: j.B.GUILD_ROLE_SUBSCRIPTIONS
                }) : (0, i.jsx)(q.A, {
                    guildId: l
                });
            case eM.VV.SERVER_MONETIZATION_ONBOARDING:
                return (0, i.jsx)(eW, {
                    guildId: l
                });
            case eM.VV.GAME_SHOP:
                return (0, i.jsx)(eY, {
                    ...e
                });
            case eM.VV.GUILD_SHOP:
                return (0, i.jsx)(eF, {
                    guildId: l,
                    productId: d,
                    initialTab: f
                });
            case eM.VV.MEMBER_APPLICATIONS:
                return (0, i.jsx)(o.rd, {
                    to: ej.BVt.CHANNEL(l, eM.VV.MEMBER_SAFETY)
                });
            case eM.VV.GUILD_HOME:
                return (0, i.jsx)(eK, {
                    guildId: l
                });
            case eM.VV.CHANNEL_BROWSER:
                return (0, i.jsx)(ez, {
                    guildId: l,
                    selectedSection: eU.qC.BROWSE
                });
            case eM.VV.GUILD_ONBOARDING:
                return (0, i.jsx)(e$, {
                    guildId: l
                });
            case eM.VV.CUSTOMIZE_COMMUNITY:
                return (0, i.jsx)(ez, {
                    guildId: l,
                    selectedSection: eU.qC.CUSTOMIZE
                });
            case eM.VV.MEMBER_SAFETY:
                return (0, i.jsx)(eq, {
                    guildId: l
                });
            case eM.VV.GUILD_BOOSTS:
                return (0, i.jsx)(eQ, {
                    guildId: l,
                    powerupListingId: n.get("powerupId") ?? n.get("powerupSkuId")
                });
            case eM.VV.GAME_SERVERS:
                if (!g || !I && !E) return (0, i.jsx)(o.rd, {
                    to: ej.BVt.CHANNEL(l)
                });
                return (0, i.jsx)(eX, {
                    guildId: l
                });
            case eM.VV.REPORT_TO_MOD:
                return (0, i.jsx)(y.A, {});
            default:
                (0, eR.xb)(u)
        }
        if ((0, Y.$Y)(_, h)) return (0, i.jsx)(W.A, {
            guildId: l,
            channelId: u
        });
        if (r?.type === ej.rbe.GUILD_STORE) {
            let e = eP.default.cast(t.params.messageId);
            return (0, i.jsx)(p.A, {
                channel: r,
                inputSkuId: e
            }, r.id)
        }
        return (0, i.jsx)(y.A, {})
    },
    eJ = a.memo(function() {
        let {
            guildId: e,
            channelId: t
        } = (0, ep.A)(), {
            notificationCenterVariant: n
        } = (0, er.X8)({
            location: "Sidebar"
        }), a = (0, A.bG)([ey.A], () => t ?? ey.A.getChannelId(e));
        return window.location.pathname.startsWith(ej.BVt.CHANNEL(ej.gNP)) ? ("sidebar" !== n && (0, eC.bG)(ex.A.defaultRoute), (0, i.jsx)(es.A, {
            includePanelSpacing: !0
        })) : window.location.pathname.startsWith(ej.BVt.GLOBAL_DISCOVERY) || window.location.pathname.startsWith(ej.BVt.GUILD_DISCOVERY) || (0, e_.r$)() ? (0, i.jsx)(B.A, {}) : null != e ? (0, i.jsx)(z.A, {
            selectedChannelId: a,
            guildId: e
        }, e) : (0, i.jsx)(eA.A, {})
    }),
    e0 = e => (0, i.jsx)(eZ, {
        ...e
    }),
    e1 = () => (0, i.jsx)(V.A, {}),
    e7 = e => (0, i.jsx)(e6, {
        ...e
    }),
    e6 = e => {
        let {
            channelId: t,
            messageId: n
        } = e.match.params;
        return (a.useEffect(() => {
            null != t && null != n && g.A.jumpToMessage({
                channelId: t,
                messageId: n,
                flash: !0,
                context: "notification_center_navigation"
            })
        }, [t, n]), null == t) ? (0, i.jsx)(el.A, {}) : e0(e)
    },
    e9 = e => {
        let {
            match: t
        } = e;
        return null != t.params.guildId ? (0, i.jsx)(F.A, {
            guildId: t.params.guildId
        }) : null
    },
    e5 = e => {
        let {
            match: t
        } = e;
        return (0, eC.pX)(ej.BVt.ME), (0, Q.A)(t.params.inviteCode), null
    },
    e2 = () => (0, i.jsx)(I.A, {}),
    e3 = () => (0, i.jsx)(f.A, {}),
    e4 = () => function(e) {
        let {
            tab: t
        } = (0, s.parse)(e.location.search);
        return Object.values(ew.G2).includes(t) ? (0, i.jsx)(R.A, {
            tab: t,
            ...e
        }) : (0, i.jsx)(R.A, {
            ...e
        })
    };

function e8() {
    return (0, i.jsx)(eh.A, {
        topLevelRoute: !0
    })
}
let te = e => (0, i.jsx)(x.A, {
        ...e
    }),
    tt = () => (0, i.jsx)(ed.A, {}),
    tn = () => (0, i.jsx)(X.A, {}),
    ti = () => (0, i.jsx)(et.A, {}),
    ta = () => (0, i.jsx)(eo.A, {}),
    tr = e => {
        let t = e.location.pathname;
        return (0, i.jsx)(C.A, {
            deepLinkType: eG.XK.FEATURES,
            path: t,
            children: (e => {
                let {
                    match: t
                } = e;
                switch (t.params.feature) {
                    case "friends":
                        return (0, i.jsx)(ed.A, {
                            initialSection: ej.m3P.ONLINE
                        });
                    case "addFriends":
                        return (0, i.jsx)(ed.A, {
                            initialSection: ej.m3P.ADD_FRIEND
                        });
                    case "pendingFriends":
                        return (0, i.jsx)(ed.A, {
                            initialSection: ej.m3P.PENDING
                        });
                    case "sessionManagement":
                        return (0, i.jsx)(o.rd, {
                            to: (0, eN.settingsPathToRoute)(ek.od.SESSIONS)
                        });
                    case "connections":
                        return (0, i.jsx)(o.rd, {
                            to: {
                                pathname: (0, eN.settingsPathToRoute)(ek.od.CONNECTIONS),
                                search: e.location.search
                            }
                        });
                    case "family-center":
                        return (0, i.jsx)(o.rd, {
                            to: ej.BVt.FAMILY_CENTER
                        });
                    case "account-standing":
                        return (0, i.jsx)(o.rd, {
                            to: (0, eN.settingsPathToRoute)(ek.od.ACCOUNT_ACCOUNT_STANDING)
                        });
                    case "authorized-apps":
                        return (0, i.jsx)(o.rd, {
                            to: (0, eN.settingsPathToRoute)(ek.od.AUTHORIZED_APPS)
                        });
                    case "open-shop":
                        let {
                            tab: n
                        } = (0, s.parse)(e.location.search);
                        if (Object.values(ew.G2).includes(n)) return (0, i.jsx)(o.rd, {
                            to: ej.BVt.COLLECTIBLES_SHOP_WITH_TAB(n)
                        });
                        return (0, i.jsx)(o.rd, {
                            to: ej.BVt.COLLECTIBLES_SHOP
                        });
                    case "editProfile":
                        return (0, i.jsx)(o.rd, {
                            to: {
                                pathname: (0, eN.settingsPathToRoute)(ek.od.PROFILE_CUSTOMIZATION)
                            }
                        });
                    case "gift":
                    case "store":
                        return (0, i.jsx)(o.rd, {
                            to: {
                                pathname: ej.BVt.NITRO_HOME
                            }
                        });
                    case "connected-games":
                        return (0, i.jsx)(o.rd, {
                            to: (0, eN.settingsPathToRoute)(ek.od.CONTENT_AND_SOCIAL_CONNECTED_GAMES)
                        });
                    case "boost":
                        let a = t.params.guildId;
                        return (0, i.jsx)(o.rd, {
                            to: ej.BVt.POWERUP_STORE(a)
                        });
                    case "boost-settings":
                        return (0, i.jsx)(o.rd, {
                            to: (0, eN.settingsPathToRoute)(ek.od.GUILD_BOOSTING)
                        });
                    default:
                        return (0, i.jsx)(o.rd, {
                            to: ej.BVt.ME
                        })
                }
            })(e)
        })
    };

function tl(e) {
    document.body.style.setProperty("--custom-guild-sidebar-width", `${e}px`)
}

function ts(e) {
    let {
        hasNotice: t,
        sidebarTheme: n,
        hideSidebar: r,
        hideChannelList: s,
        isSidebarOpen: o
    } = e, d = (0, A.bG)([b.A], () => b.A.isFullscreenInContext()), c = (0, M.D)("ChannelSidebar"), m = (0, D.useAppSidebarState)(e => !e.isOpen) && c;
    a.useLayoutEffect(() => {
        if (c) {
            let e = parseInt(_.w.get(E.o) ?? "");
            Number.isNaN(e) && (e = 375), tl(e = Math.min(Math.max(e, 264), 432));
            let t = e => {
                e.metaKey && "b" === e.key && D.useAppSidebarState.setState(e => ({
                    isOpen: !e.isOpen
                }))
            };
            return document.addEventListener("keydown", t), () => {
                document.removeEventListener("keydown", t)
            }
        }
    }, [c]), a.useLayoutEffect(() => {
        if (m) tl(76);
        else {
            let e = parseInt(_.w.get(E.o) ?? "");
            Number.isNaN(e) && (e = 375), tl(e)
        }
    }, [m]);
    let g = a.useRef(null),
        p = a.useCallback((e, t) => {
            tl(e);
            let n = t < 88;
            t <= 264 ? document.body.classList.add(eV._A) : document.body.classList.remove(eV._A), t >= 432 ? document.body.classList.add(eV.uN) : document.body.classList.remove(eV.uN);
            let i = t - e;
            g.current?.style.setProperty("--custom-overdrag", `${1+Math.min(Math.abs(i/76),.25)}`), c && (D.useAppSidebarState.setState({
                isOpen: !n
            }), n && tl(76))
        }, [c]),
        I = a.useCallback(() => {
            document.body.classList.add(eV.cB)
        }, []),
        f = a.useCallback(e => {
            document.body.classList.remove(eV.cB), document.body.classList.remove(eV._A), document.body.classList.remove(eV.uN), document.body.classList.remove(eV.l4), g.current?.style.setProperty("--custom-overdrag", "0px"), _.w.set(E.o, e), eL.default.track(ej.HAw.CHANNEL_SIDEBAR_RESIZED, {
                width: e
            })
        }, []),
        C = (0, T.A)({
            minDimension: 264,
            maxDimension: 432,
            resizableDomNodeRef: g,
            onElementResize: p,
            onElementResizeStart: I,
            onElementResizeEnd: f,
            orientation: T.R.HORIZONTAL_RIGHT,
            throttleDuration: 0
        }),
        N = a.useCallback(() => {
            D.useAppSidebarState.setState(e => ({
                isOpen: !e.isOpen
            }))
        }, []);
    if (a.useLayoutEffect(() => {
            m && tl(76)
        }, [m]), r) return null;
    let S = {
        className: l()(eV.th, {
            [eV.Qd]: !t
        })
    };
    return (0, i.jsx)(h.NPJ, {
        theme: n,
        children: e => (0, i.jsxs)("div", {
            ref: g,
            "data-collapsed": m,
            className: l()(eV.pz, e, {
                [eV.Ij]: u.Fr,
                [eV.R]: d,
                [eV.WG]: s
            }),
            children: [o && (0, i.jsx)($.A, {
                className: eV.Pl,
                themeOverride: n
            }), !s && (0, i.jsxs)(i.Fragment, {
                children: [(0, i.jsx)("div", {
                    ...S,
                    children: (0, i.jsx)(eJ, {})
                }), (0, i.jsx)(h.DUT, {
                    onClick: N,
                    "aria-label": "Resize Sidebar",
                    className: eV.tq,
                    onMouseDown: C
                }), (0, i.jsx)(eD.L, {})]
            })]
        })
    })
}

function to() {
    let e, t = (0, A.bG)([eO.Ay], () => eO.Ay.hasNotice()),
        n = (0, eT.NC)(),
        r = function() {
            let [e, t] = a.useState(() => window.location.pathname.startsWith(ej.BVt.GUILD_MEMBER_VERIFICATION("")));
            return a.useEffect(() => eE.A.addRouteChangeListener(e => {
                t(e.pathname.startsWith(ej.BVt.GUILD_MEMBER_VERIFICATION("")))
            }), []), e
        }(),
        l = (e = (0, o.zy)(), (0, ef.B)(e.pathname, ej.BVt.GUILD_BOOSTING_MARKETING(eI.pv.guildId()))?.isExact === !0),
        s = (0, A.bG)([ei.A], () => ei.A.getIsOpen()),
        {
            notificationCenterVariant: c
        } = (0, er.X8)({
            location: "Sidebar"
        }),
        _ = (0, w.f)("AppView"),
        h = (0, o.W5)([ej.BVt.CHANNEL(ej.ME, eI.pv.channelId()), ej.BVt.CHANNEL(eI.pv.guildId(), eI.pv.channelId({
            optional: !0
        }), ":messageId?")]),
        m = h?.params?.channelId === eM.VV.GUILD_ONBOARDING,
        g = a.useCallback(() => en.A.openSidebar(), []),
        p = (0, M.D)("AppView"),
        E = (0, D.useAppSidebarState)(e => !e.isOpen),
        I = (0, A.bG)([b.A], () => b.A.isFullscreenInContext()),
        f = (0, A.bG)([G.A], () => G.A.isFrameActive());
    return (0, i.jsx)("div", {
        className: eV.kL,
        children: (0, i.jsx)(H.A.Provider, {
            value: h?.params.guildId === ej.ME ? void 0 : h?.params.guildId,
            children: (0, i.jsxs)("div", {
                className: eV.E3,
                "data-fullscreen": I,
                children: [!I && (0, i.jsx)(eB, {}), u.Fr ? null : (0, i.jsx)(ea.A, {}), (0, i.jsxs)("div", {
                    className: eV.Qs,
                    children: [(0, i.jsx)(O.A, {}), (0, i.jsx)(ts, {
                        isSidebarOpen: s,
                        hasNotice: t,
                        sidebarTheme: n,
                        hideChannelList: r || l || m,
                        hideSidebar: !s
                    }), (0, i.jsx)("div", {
                        className: eV.MY,
                        "data-collapsed": !!p && E,
                        children: (0, i.jsxs)(S.A.Provider, {
                            value: g,
                            children: [(0, i.jsxs)(o.dO, {
                                children: [(0, i.jsx)(v.A, {
                                    path: ej.BVt.ACTIVITY,
                                    disableTrack: !0,
                                    children: (0, i.jsx)(o.rd, {
                                        to: ej.BVt.ME
                                    })
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.ACTIVITY_DETAILS(":applicationId"),
                                    render: te,
                                    impressionName: d.ImpressionNames.ACTIVITY_DETAILS,
                                    impressionProperties: e => {
                                        let {
                                            match: t
                                        } = e;
                                        return {
                                            application_id: t?.params.applicationId
                                        }
                                    }
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.APPLICATION_LIBRARY,
                                    render: e2,
                                    impressionName: d.ImpressionNames.APPLICATION_LIBRARY,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.APPLICATION_STORE,
                                    render: e3,
                                    impressionName: d.ImpressionNames.APPLICATION_STORE,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.COLLECTIBLES_SHOP_WITH_TAB(":tab"),
                                    render: e4(),
                                    disableTrack: !0
                                }), _ && (0, i.jsx)(v.A, {
                                    path: ej.BVt.COLLECTIBLES_SHOP_LAYOUT(":layoutId"),
                                    render: function(e) {
                                        let {
                                            match: t,
                                            ...n
                                        } = e, {
                                            layoutId: a
                                        } = t.params;
                                        return null == a || "" === a ? (0, i.jsx)(R.A, {
                                            ...n
                                        }) : (0, i.jsx)(P.A, {
                                            layoutId: a,
                                            ...n
                                        })
                                    },
                                    disableTrack: !0
                                }), _ && (0, i.jsx)(v.A, {
                                    path: ej.BVt.COLLECTIBLES_SHOP_COLLECTION_DETAIL(":collectionId"),
                                    render: function(e) {
                                        let {
                                            match: t,
                                            ...n
                                        } = e, {
                                            collectionId: a
                                        } = t.params;
                                        return null == a || "" === a ? (0, i.jsx)(R.A, {
                                            ...n
                                        }) : (0, i.jsx)(L.A, {
                                            collectionId: a,
                                            ...n
                                        })
                                    },
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.COLLECTIBLES_SHOP,
                                    render: e4(),
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.COLLECTIBLES_SHOP_PRODUCT_DETAIL(":skuId"),
                                    render: e4(),
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.ICYMI,
                                    render: tn,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.MESSAGE_REQUESTS,
                                    render: ti,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.FAMILY_CENTER,
                                    render: ta,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.ME,
                                    exact: !0,
                                    render: tt,
                                    impressionName: d.ImpressionNames.FRIENDS,
                                    disableTrack: !0
                                }), c === er.U5.SIDEBAR && (0, i.jsx)(v.A, {
                                    path: ej.BVt.CHANNEL(ej.gNP, eI.pv.channelId({
                                        optional: !0
                                    }), ":messageId?"),
                                    render: e7,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: [ej.BVt.CHANNEL_THREAD_VIEW(eI.pv.guildId(), eI.pv.channelId(), ":threadId", ":messageId?"), ej.BVt.CHANNELS_GAME_SHOP(eI.pv.guildId(), ":gameShopPageIndex", ":gameShopSkuId?", ":gameShopSlug?"), ej.BVt.CHANNEL(ej.ME, eI.pv.channelId()), ej.BVt.CHANNEL(eI.pv.guildId(), eI.pv.channelId({
                                        optional: !0
                                    }), ":messageId?")],
                                    render: e0,
                                    impressionName: d.ImpressionNames.GUILD_CHANNEL,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GLOBAL_DISCOVERY,
                                    render: e1,
                                    impressionName: d.ImpressionNames.GLOBAL_DISCOVERY,
                                    disableTrack: !0,
                                    exact: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GLOBAL_DISCOVERY_SERVERS,
                                    render: e1,
                                    impressionName: d.ImpressionNames.GLOBAL_DISCOVERY,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GLOBAL_DISCOVERY_APPS,
                                    render: e1,
                                    impressionName: d.ImpressionNames.GLOBAL_DISCOVERY_APPS,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.QUEST_HOME_DEPRECATED,
                                    render: e1,
                                    impressionName: d.ImpressionNames.QUEST_HOME,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.QUEST_HOME,
                                    render: e8,
                                    impressionName: d.ImpressionNames.QUEST_HOME,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GUILD_DISCOVERY,
                                    render: e1,
                                    impressionName: d.ImpressionNames.GUILD_DISCOVERY,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GUILD_MEMBER_VERIFICATION(eI.pv.guildId()),
                                    render: e9,
                                    impressionName: d.ImpressionNames.GUILD_MEMBER_VERIFICATION,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GUILD_MEMBER_VERIFICATION_FOR_HUB(eI.pv.guildId(), ":inviteCode?"),
                                    render: e5
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.GUILD_FEATURE(":feature", eI.pv.guildId()),
                                    exact: !0,
                                    render: tr,
                                    disableTrack: !0
                                }), (0, i.jsx)(v.A, {
                                    path: ej.BVt.FEATURE(":feature"),
                                    exact: !0,
                                    render: tr,
                                    disableTrack: !0
                                })]
                            }), f && (0, i.jsx)(k.A, {})]
                        })
                    })]
                })]
            })
        })
    })
}