/** chunk id: 158265 params = (module,exports,require) **/
n.d(t, {
    A: () => A
});
var a = n(627968),
    i = n(64700),
    s = n(503698),
    l = n.n(s),
    r = n(106778),
    o = n(340287),
    d = n(687498),
    c = n(311907),
    u = n(475743),
    m = n(775602),
    h = n(186111),
    x = n(398025),
    p = n(568329),
    g = n(963713),
    _ = n(772244),
    f = n(272111),
    v = n(851110),
    b = n(131206),
    j = n(231901);
let A = e => {
    let {
        overlayRef: t,
        progressBarRef: n,
        isHovered: s
    } = e, {
        quest: A,
        isExpanded: C
    } = i.useContext(g.T), {
        expansionSpring: y
    } = i.useContext(p.PW), {
        completionSpring: E,
        startCompletionAnimation: S
    } = (0, _.R)(), T = A.userStatus?.completedAt != null, N = i.useRef(!1), I = (0, c.bG)([m.A], () => m.A.useReducedMotion), O = i.useRef(null), k = (0, c.bG)([h.A], () => h.A.hasLayers()), R = (0, u.A)(k), [w, D] = i.useState(null), [M, P] = i.useState(null), L = i.useRef(new r.OH({
        gravity: 0,
        wind: 0
    })), U = (0, r.f9)(w, M), B = i.useMemo(() => [f.r3], []), G = i.useCallback(() => {
        if (I) return;
        let e = n.current,
            t = O.current;
        if (null != t && null != e && U.isReady) {
            var a, i, s, l;
            let {
                x: n,
                y: r
            } = e.getBoundingClientRect(), {
                x: o,
                y: d
            } = t.getBoundingClientRect();
            U.createMultipleConfetti((a = n - o, i = r - d, s = e.clientHeight, l = e.clientWidth, {
                ...v.Mw,
                position: {
                    type: "static-random",
                    minValue: {
                        x: a,
                        y: i
                    },
                    maxValue: {
                        x: a + s,
                        y: i + l
                    }
                },
                velocity: {
                    type: "static-random",
                    minValue: {
                        x: -20,
                        y: -20
                    },
                    maxValue: {
                        x: 20,
                        y: 20
                    }
                },
                opacity: {
                    type: "linear",
                    value: 2,
                    addValue: -.1
                },
                dragCoefficient: {
                    type: "static",
                    value: .166
                },
                size: {
                    type: "static-random",
                    minValue: 2,
                    maxValue: 3
                }
            }), 100)
        }
    }, [n, O, U, I]), F = (0, u.A)(C);
    return (i.useEffect(() => {
        T && C && !F && (S(), G())
    }, [C, T, S, G, F]), i.useEffect(() => {
        T && !k && R && setTimeout(() => {
            S(), G()
        }, 200)
    }, [T, R, k, S, G]), i.useEffect(() => {
        U.isReady && (!N.current && T && (S(), G()), N.current = T)
    }, [T, N, G, S, U]), i.useEffect(() => {
        s && T && (S(), G())
    }, [s, T, S, G]), I) ? null : (0, a.jsxs)("div", {
        className: b.iE,
        "aria-hidden": "true",
        ref: O,
        children: [(0, a.jsx)(d.animated.div, {
            className: b.Tp,
            style: {
                opacity: (0, x.a)(E)
            }
        }), (0, a.jsx)(d.animated.div, {
            className: l()(b.sJ, b.ix),
            style: {
                opacity: (0, x.a)(E)
            }
        }), (0, a.jsxs)(d.animated.div, {
            className: b.KG,
            style: {
                transform: y.to({
                    range: [0, 1],
                    output: [-35, 0]
                }).to(e => `translateY(${e}px)`)
            },
            children: [(0, a.jsx)(r.Fk, {
                ref: D,
                className: b.t_,
                environment: L.current
            }), (0, a.jsx)(r.K_, {
                ref: P,
                sprites: [j],
                colors: B,
                spriteWidth: v.wn,
                spriteHeight: v.wn
            }), null != t.current && (0, o.createPortal)((0, a.jsx)(d.animated.div, {
                className: l()(b.sJ, b.d7),
                style: {
                    opacity: (0, x.a)(E)
                }
            }), t.current)]
        })]
    })
}