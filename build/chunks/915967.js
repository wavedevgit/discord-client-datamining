/** chunk id: 915967 params = (module,exports,require) **/
n.d(t, {
    Ay: () => q,
    Bx: () => X,
    Gm: () => k,
    Q_: () => w,
    zF: () => K,
    zR: () => Y
}), n(321073);
var i, r = n(966597),
    l = n(774603),
    s = n(430086),
    o = n(122032),
    a = n(394318),
    d = n(303757),
    c = n(830004),
    u = n(434728),
    _ = n(308494),
    A = n(64460),
    E = n(92960),
    I = n(739008),
    g = n(314519),
    h = n(198168),
    C = n(799592),
    p = n(675704),
    S = n(806964),
    T = n(552049),
    b = n(877991),
    N = n(332779),
    m = n(187521),
    L = n(568765),
    f = n(904828),
    O = n(251494),
    G = n(82038),
    R = n(14214),
    U = n(151199),
    D = n(144353),
    y = n(535527),
    P = n(46647),
    M = n(512513),
    x = n(910492),
    v = n(416092),
    j = n(801796),
    V = n(344731),
    H = n(58841),
    B = n(406975),
    W = n(652215),
    F = n(985018),
    w = ((i = {}).NAVIGATION = "NAVIGATION", i.CHAT = "CHAT", i.VOICE_AND_VIDEO = "VOICE_AND_VIDEO", i.MISCELLANEOUS = "MISCELLANEOUS", i.MESSAGE = "MESSAGE", i.DND = "DND", i);

function k(e) {
    switch (e) {
        case "NAVIGATION":
            return F.intl.string(F.t["yGE+jg"]);
        case "VOICE_AND_VIDEO":
            return F.intl.string(F.t.bI8F5u);
        case "CHAT":
            return F.intl.string(F.t.hDhbb3);
        case "MISCELLANEOUS":
            return F.intl.string(F.t.cBdwqs);
        case "MESSAGE":
            return F.intl.string(F.t["5fpmX9"]);
        case "DND":
            return F.intl.string(F.t["69j6+4"])
    }
}

function K(e) {
    switch (e) {
        case "MESSAGE":
            return F.intl.string(F.t.iepGDn);
        case "DND":
            return F.intl.string(F.t.LBsB0a);
        default:
            return
    }
}

function z() {
    for (var e = arguments.length, t = Array(e), n = 0; n < e; n++) t[n] = arguments[n];
    return t.map(e => {
        let t = Y[e];
        if (null == t) throw Error(`getBindsFor(...): No bind for ${t}`);
        return t.binds[0]
    })
}
let Y = {
    [W.IWg.SERVER_NEXT]: f.yx,
    [W.IWg.SERVER_PREV]: f.yv,
    [W.IWg.CHANNEL_NEXT]: s.kF,
    [W.IWg.CHANNEL_PREV]: s.Oc,
    [W.IWg.NAVIGATE_BACK]: s.GY,
    [W.IWg.NAVIGATE_FORWARD]: s.M$,
    [W.IWg.UNREAD_NEXT]: j.mH,
    [W.IWg.UNREAD_PREV]: j.US,
    [W.IWg.MENTION_CHANNEL_NEXT]: j.BD,
    [W.IWg.MENTION_CHANNEL_PREV]: j.X8,
    [W.IWg.TOGGLE_PREVIOUS_GUILD]: s.Fv,
    [W.IWg.JUMP_TO_GUILD]: A.J,
    [W.IWg.SUBMIT]: O.X,
    [W.IWg.TEXTAREA_FOCUS]: G.c,
    [W.IWg.MARK_CHANNEL_READ]: E.Df,
    [W.IWg.MARK_SERVER_READ]: I.P,
    [W.IWg.TOGGLE_CHANNEL_PINS]: D.O,
    [W.IWg.TOGGLE_FOR_LATER]: y.K,
    [W.IWg.TOGGLE_INBOX]: x.q,
    [W.IWg.MARK_TOP_INBOX_CHANNEL_READ]: x.O,
    [W.IWg.TOGGLE_USERS]: v.U,
    [W.IWg.TOGGLE_HELP]: P.x,
    [W.IWg.VIBE_WITH_WUMPUS]: H.y,
    [W.IWg.TOGGLE_MUTE]: R.VT,
    [W.IWg.TOGGLE_DEAFEN]: R.rR,
    [W.IWg.TOGGLE_CATEGORY_COLLAPSED]: U.y,
    [W.IWg.SEARCH_SOUNDBOARD]: m.A,
    [W.IWg.SCROLL_UP]: T.U5,
    [W.IWg.SCROLL_DOWN]: T.fz,
    [W.IWg.QUICKSWITCHER_SHOW]: p.R,
    [W.IWg.CREATE_DM_GROUP]: a.X,
    [W.IWg.SEARCH_EMOJIS]: b.L,
    [W.IWg.SEARCH_GIFS]: N.T,
    [W.IWg.SEARCH_STICKERS]: L.w,
    [W.IWg.TOGGLE_HOTKEYS]: M.z,
    [W.IWg.JUMP_TO_FIRST_UNREAD]: g.s,
    [W.IWg.CREATE_GUILD]: d.$,
    [W.IWg.UPLOAD_FILE]: V.T,
    [W.IWg.RETURN_TO_AUDIO_CHANNEL]: S.u,
    [W.IWg.CALL_ACCEPT]: l.Yo,
    [W.IWg.CALL_START]: l.OX,
    [W.IWg.FOCUS_SEARCH]: u.O,
    [W.IWg.JUMP_TO_CURRENT_CALL]: _.s,
    [W.IWg.ZOOM_IN]: B.Ur,
    [W.IWg.ZOOM_OUT]: B.hU,
    [W.IWg.ZOOM_RESET]: B.O$,
    [W.IWg.OPEN_APP_DIRECTORY]: h.N,
    [W.IWg.BROWSER_DEVTOOLS]: c.i,
    [W.IWg.OPEN_CONTEXT_MENU]: C.b,
    [W.IWg.COPY_CHANNEL_LINK]: o.F
};

function X() {
    let e = [{
            description: F.intl.string(F.t.bx4Uyz),
            binds: z(W.IWg.SERVER_PREV, W.IWg.SERVER_NEXT),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t["+Wem6h"]),
            binds: z(W.IWg.CHANNEL_PREV, W.IWg.CHANNEL_NEXT),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t["+2fcdz"]),
            binds: z(W.IWg.NAVIGATE_BACK, W.IWg.NAVIGATE_FORWARD),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t.eVmj1H),
            binds: z(W.IWg.UNREAD_PREV, W.IWg.UNREAD_NEXT),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t.EcqS7Y),
            binds: z(W.IWg.MENTION_CHANNEL_PREV, W.IWg.MENTION_CHANNEL_NEXT),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t["4I3pwW"]),
            binds: z(W.IWg.JUMP_TO_CURRENT_CALL),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t.Bqss72),
            binds: z(W.IWg.TOGGLE_PREVIOUS_GUILD),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t.yYsRlD),
            binds: z(W.IWg.QUICKSWITCHER_SHOW),
            group: "NAVIGATION"
        }, {
            description: F.intl.string(F.t.O7ouXO),
            binds: z(W.IWg.CREATE_GUILD),
            group: "NAVIGATION",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.Lns0Fc),
            binds: ["mod+d"],
            group: "DND"
        }, {
            description: F.intl.string(F.t.dmMqay),
            binds: ["up", "down"],
            group: "DND"
        }, {
            description: F.intl.string(F.t["cs/HVH"]),
            binds: ["spacebar", "enter"],
            group: "DND"
        }, {
            description: F.intl.string(F.t["1ioMJQ"]),
            binds: ["esc"],
            group: "DND",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.UaXAPx),
            binds: z(W.IWg.MARK_SERVER_READ),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["5X9vFj"]),
            binds: z(W.IWg.MARK_CHANNEL_READ),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.wxQFsl),
            binds: z(W.IWg.CREATE_DM_GROUP),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["C+XV7f"]),
            binds: z(W.IWg.TOGGLE_CHANNEL_PINS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["Q+YV/T"]),
            binds: z(W.IWg.TOGGLE_INBOX),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["YEjV+W"]),
            binds: z(W.IWg.MARK_TOP_INBOX_CHANNEL_READ),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.AcBI9S),
            binds: z(W.IWg.TOGGLE_USERS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.JoxNnl),
            binds: z(W.IWg.SEARCH_EMOJIS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["3PHxo8"]),
            binds: z(W.IWg.SEARCH_GIFS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.YFl7eb),
            binds: z(W.IWg.SEARCH_STICKERS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.L3RYYJ),
            binds: z(W.IWg.SCROLL_UP, W.IWg.SCROLL_DOWN),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["3HAurM"]),
            binds: z(W.IWg.JUMP_TO_FIRST_UNREAD),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.rrYBEu),
            binds: z(W.IWg.TEXTAREA_FOCUS),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t.sUJlPL),
            binds: z(W.IWg.UPLOAD_FILE),
            group: "CHAT"
        }, {
            description: F.intl.string(F.t["A+Fv0R"]),
            binds: z(W.IWg.COPY_CHANNEL_LINK),
            group: "CHAT",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.tL6eVW),
            binds: z(W.IWg.TOGGLE_MUTE),
            group: "VOICE_AND_VIDEO"
        }, {
            description: F.intl.string(F.t["QXe/7T"]),
            binds: z(W.IWg.TOGGLE_DEAFEN),
            group: "VOICE_AND_VIDEO"
        }, {
            description: F.intl.string(F.t.d6UIii),
            binds: z(W.IWg.CALL_ACCEPT),
            group: "VOICE_AND_VIDEO"
        }, {
            description: F.intl.string(F.t.IcEW06),
            binds: z(W.IWg.MARK_CHANNEL_READ),
            group: "VOICE_AND_VIDEO"
        }, {
            description: F.intl.string(F.t.WN2dsS),
            binds: z(W.IWg.CALL_START),
            group: "VOICE_AND_VIDEO",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.rUK0kk),
            binds: z(W.IWg.SEARCH_SOUNDBOARD),
            group: "VOICE_AND_VIDEO"
        }, {
            description: F.intl.string(F.t.vkGkSn),
            binds: z(W.IWg.TOGGLE_HELP),
            group: "MISCELLANEOUS"
        }, {
            description: F.intl.string(F.t.FJvZ87),
            binds: z(W.IWg.FOCUS_SEARCH),
            group: "MISCELLANEOUS"
        }, {
            description: F.intl.string(F.t["FiWl/T"]),
            binds: z(W.IWg.OPEN_CONTEXT_MENU),
            group: "MISCELLANEOUS"
        }, {
            description: F.intl.string(F.t.HnNtEI),
            binds: ["h+h+right+n+k"],
            group: "MISCELLANEOUS",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.fsBWmS),
            binds: ["e"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.xwMqD7),
            binds: ["backspace"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.CvQ18w),
            binds: ["p"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.lfIHs4),
            binds: ["plus"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t["5IEsGx"]),
            binds: ["r"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.zSyDdA),
            binds: ["f"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.yGLjXF),
            binds: ["s"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.JrGD7E),
            binds: ["mod+c"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.RpE9k7),
            binds: ["alt+enter"],
            group: "MESSAGE"
        }, {
            description: F.intl.string(F.t.rrYBEu),
            binds: ["escape"],
            group: "MESSAGE",
            groupEnd: !0
        }, {
            description: F.intl.string(F.t.z9c6mt),
            binds: z(W.IWg.VIBE_WITH_WUMPUS),
            group: "MISCELLANEOUS"
        }],
        {
            enabled: t,
            inInbox: n
        } = r.A.getConfig({
            location: "keybinds"
        });
    return t && !n && e.push({
        description: F.intl.string(F.t.IWNSoH),
        binds: z(W.IWg.TOGGLE_FOR_LATER),
        group: "CHAT"
    }), e
}
let q = 21552 == n.j ? Y : null