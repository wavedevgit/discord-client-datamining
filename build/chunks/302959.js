...
let v = a.A.Types,
    I = function(t) {
        let {
            ...
            ...I
        } = t, _ = (0, r.bG)([g.A, h.A], () => h.A.getChannel(g.A.getVoiceStateForUser(i.id)?.channelId)), x = (0, c.v)("UserActivityContainer", _), N = (0, r.bG)([A.A], () => o ? A.A.getAnyStreamForUser(i.id) : null), y = e?.type === S.$pd.HANG_STATUS && x ? _ : null, j = (0, r.bG)([p.A, g.A, h.A], () => (0, s.A)(e, S.jUm.EMBEDDED) ? p.A.getGuild(h.A.getChannel(g.A.getVoiceStateForSession(i.id, e?.session_id)?.channelId)?.getGuildId()) : null != y ? p.A.getGuild(y.getGuildId()) : null), D = (0, r.bG)([p.A], () => null != N ? p.A.getGuild(N.guildId) : null), O = (0, r.bG)([d.A], () => {
            if (null != e)
        ...
        return (l.useEffect(() => {
            e?.type === S.$pd.HANG_STATUS && x && f.default.track(S.HAw.VIEW_HANG_STATUS, {
                source: "UserActivity",
                other_user_id: i.id,
                ...(0, u.A)(y?.id)
            })
        }, [e?.type, x, y, i.id]), e?.type !== S.$pd.HANG_STATUS || x) ? (0, n.jsx)(a.A, {
            ...I,
            activity: e,
            user: i,
            application: O,
