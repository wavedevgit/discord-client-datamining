/** chunk id: 120385, original params: e,t,n (module,exports,require) **/
n.d(t, {
    A: () => m
});
var i = n(627968),
    s = n(64700),
    a = n(562465),
    l = n(397927),
    r = n(279280),
    o = n(652215),
    c = n(818348),
    d = n(985018),
    u = n(956171);
async function _(e) {
    try {
        return (await a.Bo.get({
            url: o.Rsh.BILLING_INVOICE_BREAKDOWN,
            query: {
                payment_id: e
            },
            oldFormErrors: !0,
            rejectWithError: !1
        })).body
    } catch (e) {
        throw e
    }
}

function m(e) {
    let {
        payment: t
    } = e, [n, a] = s.useState(null), [o, m] = s.useState(null), A = async e => {
        try {
            let n = await _(t.id);
            a(n);
            let i = e ? n.refundInvoiceLinks[0] : n.invoiceLink;
            window.open(i, "_blank"), m(null)
        } catch (e) {
            m(e.body?.message)
        }
    }, g = null != t.paymentSource && t.status === c.__.COMPLETED, h = n?.invoiceLink, x = n?.refundInvoiceLinks;
    return t.hasInvoiceURL && null == n ? (0, i.jsxs)("div", {
        className: u.It,
        children: [(0, i.jsx)(l.MzZ, {
            onClick: () => A(!1),
            children: d.intl.formatToPlainString(d.t.R0xzCN, {})
        }), t.hasRefundInvoiceURLs ? (0, i.jsx)(l.MzZ, {
            className: u.oe,
            onClick: () => A(!0),
            children: d.intl.formatToPlainString(d.t["3x6NGw"], {})
        }) : null, null != o && "" !== o && (0, i.jsx)(l.wx6, {
            type: "critical",
            children: o
        })]
    }) : t.hasInvoiceURL && null != n ? (0, i.jsxs)("div", {
        className: u.It,
        children: [(0, i.jsx)(l.MzZ, {
            href: h,
            children: d.intl.formatToPlainString(d.t.R0xzCN, {})
        }), null != x ? x.map((e, t) => (0, i.jsx)(l.MzZ, {
            className: u.oe,
            href: e,
            children: d.intl.formatToPlainString(d.t["3x6NGw"], {})
        }, t)) : null, null != o && "" !== o && (0, i.jsx)(l.wx6, {
            type: "critical",
            children: o
        })]
    }) : g ? (0, i.jsx)("div", {
        className: u.It,
        children: (0, i.jsx)(l.MzZ, {
            onClick: () => {
                var e;
                e = t.paymentSource, r.A.open(t, e)
            },
            children: d.intl.formatToPlainString(d.t.onRIxS, {})
        })
    }) : null
}