...
let y = r.memo(function(e) {
    let {
        ...
        onClose: v,
        children: E
    } = e, M = r.useRef(null), C = r.useRef([]), w = r.useRef(!1), D = r.useRef(null), [N, O] = r.useState(0), [T, b] = r.useState({
        x: 0,
        y: 0
    }), j = Math.abs(T.x) + Math.abs(T.y) > 0, S = r.useMemo(() => s().chunk(E, x), [E]), k = r.useCallback((e, t) => {
        null == C.current[N] ? C.current[N] = [] : C.current[N][t] = e
    }, [N]), R = r.useCallback((e, t) => {
        D.current = t, m(x * e + t)
    }, [m]), L = r.useCallback(() => {
        D.current = null, m(null)
    }, [m]), U = r.useCallback(e => {
        L(), w.current = e
    }, [L]), P = r.useCallback((e, t, l) => {
        let n, r, a;
        if (w.current) return void b({
            x: 0,
            y: 0
        });
        let s = {
            ...
            i = s.x < 0,
            u = s.y < 0,
            c = (n = Math.abs(s.x), r = 180 / Math.PI * Math.atan2(Math.abs(s.y), n), {
    ...
    }, [_, N]), V = r.useMemo(() => (0, a.throttle)(e => {
        ...
        L()
    }, 16), [A, P, L, R, N, l, t]), Y = r.useCallback(e => {
        if (!I) return;
        let t = N + (e.deltaY > 0 ? 1 : -1);
        t >= 0 && t < S.length && (null != D.current && (S[t].length > D.current ? R(t, D.current) : L()), O(t))
    }, [I, N, S, R, L]), F = r.useMemo(() => S[N].map((e, r) => {
        let a = h[r];
