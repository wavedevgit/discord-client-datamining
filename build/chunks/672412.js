...
class f {
    ...
    start(e, t) {
        this.onMLResult = e, this.onMLModuleLoaded = t, h.info("ML signal manager started"), (0, d.TD)() && (h.info("Clips enabled on startup, initializing ML module"), this.maybeSetupMLModulePrefetchModels())
    }
    stop() {
    ...
    async maybeSetupMLModulePrefetchModels() {
        if (!(this.isNativeModuleLoaded() || this.isNativeModuleLoading()) && !__OVERLAY__) {
            ...
            try {
                this.state.nativeMLModuleState = r.Q.LOADING, await this.refreshMLCatalog(), await this.prefetchMLModels(), await this.loadMLNativeModule()
            } catch (e) {
    ...
    async loadMLNativeModule() {
        try {
            ...
            t.setupMLPath?.(), await e.setMLResultCallback(e => {
                let t;
                try {
                    t = JSON.parse(e)
                } catch (e) {
            ...
            }), this.onMLModuleLoaded?.(), this.state.nativeMLModuleState = r.Q.LOADED;
            ...
            a.setClipsMLPipelineTypeEnabled("emotion_classifier", i.emotionClassifier), a.setClipsMLPipelineTypeEnabled("wake_word_detector", i.wakeWordDetector), a.setClipsMLPipelineTypeEnabled("yell_detector", i.yellDetector), a.setClipsMLPipelineTypeEnabled("whisper_transcription", i.whisperTranscription), a.setClipsMLPipelineTypeEnabled("laughter_shouting", i.laughterDetector), h.info("ML native module loaded successfully")
        } catch (e) {
            h.warn(`Failed to load Clips ML module: ${e.message}`), this.state.nativeMLModuleState = r.Q.FAILED
        }
    }
    ...
    loadMLCatalog() {
        ...
        for (let [t, a] of c.Xg) e.models[t] = {
            url: `https://cdn.discordapp.com/assets/content/${a}.onnx`,
            version: "1.0.0"
        };
    ...
    async prefetchMLModels() {
        ...
        for (let [a, i] of Object.entries(e)) {
            ...
            e?.status !== r.D.DOWNLOADED && e?.status !== r.D.DOWNLOADING && t.push(this.downloadMLModel({
                modelId: a,
                url: i.url,
                fileName: a
            }))
    ...
    downloadMLModel(e) {
        ...
        let n = s.Ay.downloadMLModelFile(t, i, e => {
            ...
            e.fetchedFromNetwork && h.info("Downloaded ML model from network:", a), this.state.modelState[a] = {
                status: r.D.DOWNLOADED,
                downloadedBytes: void 0
            }
        }).catch(t => {
            t?.USER_CANCELED_DOWNLOAD ? h.info("User canceled the download for ML model", e) : h.error("Failed to download ML model", {
                reason: t,
                ...e
            }), this.state.modelState[a] = {
                status: r.D.MISSING
