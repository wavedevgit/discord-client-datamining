/** chunk id: 483768 params = (module,exports,require) **/
n.d(t, {
    A: () => p
});
var i = n(627968);
n(64700);
var l = n(311907),
    s = n(397927),
    r = n(843472),
    a = n(969043),
    o = n(406704),
    c = n(576705),
    d = n(287809),
    u = n(143413),
    E = n(145530),
    A = n(652215),
    m = n(985018);

function p(e, t) {
    let n = (0, l.bG)([d.default], () => d.default.getCurrentUser()),
        p = (0, o.Id)(t),
        f = (0, l.bG)([c.A], () => c.A.can(A.xBc.MANAGE_MESSAGES, t), [t]),
        {
            firstMessage: D
        } = (0, l.bG)([a.A], () => a.A.getMessage(t.id), [t.id]),
        g = f || null != n && e.canDeleteOwnMessage(n.id);
    return e.state === A.cmJ.SENDING || !g || A.MRS.UNDELETABLE.has(e.type) || !p || t.isModeratorReportChannel() && (e.id === D?.id || (0, u.A)(e)) ? null : (0, i.jsx)(s.Drp, {
        id: "delete",
        label: m.intl.string(m.t.xwMqD7),
        action: function(n) {
            e.state === A.cmJ.SEND_FAILED ? r.A.deleteMessage(t.id, e.id, !0) : n.shiftKey ? r.A.deleteMessage(t.id, e.id) : E.A.confirmDelete(t, e, !0)
        },
        color: "danger",
        leadingAccessory: {
            type: "icon",
            icon: s.ucK
        },
        icon: s.ucK
    })
}