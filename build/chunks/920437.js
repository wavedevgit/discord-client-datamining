    ...
    render() {
        ...
        if (e && !m) {
            if (N.message = H.intl.string(H.t.Hl0Mqh), null != A && A >= 1e3) {
                ...
                N.subtitle = H.intl.formatToPlainString(H.t.C5bgrC, {
                    count: e.toLocaleString()
                })
            }
            N.buttonText = H.intl.string(H.t["3aOv+h"]), N.onButtonClick = this.handleFollowAnnouncement, t && (N.onSecondaryButtonClick = this.handleJoinServer, N.secondaryButtonText = H.intl.string(H.t.RLch70))
        } else if (n) N.message = H.intl.string(H.t["Eg3/c9"]), N.buttonText = H.intl.string(H.t.fiNVin), N.onButtonClick = p ? this.handleShowMemberVerification : this.handleClaimAccount;
        else if (p) switch (x?.applicationStatus) {
            case E.B5.SUBMITTED:
                N.message = H.intl.string(H.t["5iLvSx"]), N.subtitle = H.intl.string(H.t.FdsK4h), N.buttonText = H.intl.string(H.t.mqtdmQ), N.onButtonClick = this.handleCancelApplication;
                break;
            case E.B5.REJECTED:
                N.message = H.intl.string(H.t.lk30cY), N.buttonText = H.intl.string(H.t["8RrsHr"]), N.onButtonClick = this.handleViewApplicationRejection;
                break;
            default:
                N.message = H.intl.string(H.t.rEBKvg), N.buttonText = H.intl.string(H.t["r8/DT+"]), N.buttonVariant = "primary", N.onButtonClick = this.handleShowMemberVerification
        } else C || null == S ? l && !_ ? (N.message = H.intl.string(H.t["2dThMM"]), N.buttonText = H.intl.string(H.t["50gfOv"]), N.onButtonClick = this.handleVerifyPhone) : s ? (N.message = H.intl.string(H.t.FkGPS5), N.buttonText = H.intl.string(H.t.lm1UKt), N.onButtonClick = this.handleResendVerification) : a ? (N.message = H.intl.formatToPlainString(H.t.IH7RMF, {
            min: G.$8o.MEMBER_AGE
        }), N.countdown = r) : o && (N.message = H.intl.formatToPlainString(H.t["2JA2GH"], {
            min: G.$8o.ACCOUNT_AGE
        }), N.countdown = c) : (N.imageSrc = S.getIconURL(F.iu.SMALL) ?? void 0, N.message = H.intl.format(H.t.EvDn1D, {
            name: S.name
        }), null != S.connectionEntrypointUrl && (N.buttonText = H.intl.string(H.t.S0W8Z5), N.onButtonClick = () => {
