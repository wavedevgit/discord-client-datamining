...
function k() {
    let {
        onPrimaryAction: e,
        onContextMenu: t
    } = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {}, [n, r] = i.useState(""), _ = n.trim().toLowerCase(), [O, k] = i.useState(() => new Set), P = (0, a.bG)([C.A], () => C.A.getCollapsedSectionOverridesForTab(T.x.VOICE), []), j = (0, h.qZ)(), V = i.useMemo(() => {
        let e = new Map;
    ...
    }, []), W = i.useMemo(() => new Set(j.map(e => e.channel.id)), [j]), [z, B] = i.useState([]);
    (0, d.RT)(n, j, B, h.MV);
    let $ = i.useRef(null),
        ...
        el = (0, a.bG)([v.A], () => v.A.getActiveNowChannelIds({
            kind: v.u.Voice
        }), []),
        ei = (0, a.bG)([A.A], () => A.A.affinities.filter(e => (e.score ?? 0) > 0).slice(0, 2).map(e => e.guildId), []),
        er = i.useMemo(() => {
            let e, t, n, l;
            return t = String((e = new Date).getDate()).padStart(2, "0"), n = String(e.getMonth() + 1).padStart(2, "0"), l = String(e.getFullYear() % 100).padStart(2, "0"), b(`${t}${n}${l}`)
        }, []),
        ...
        }, [eh]),
        {
            ...
            for (let l of ef) {
                ...
                for (let i of function(e, t) {
                        let n = Array.from(new Set((t ?? Object.values(m.A.getVoiceStatesForChannel(e))).map(e => e.userId)));
                        return n.sort((e, t) => {
                            let n = f.A.isFriend(e),
                                l = f.A.isFriend(t);
                            return n && !l ? -1 : !n && l ? 1 : (c.A.getUserAffinity(t)?.vcProbability ?? 0) - (c.A.getUserAffinity(e)?.vcProbability ?? 0)
                        }), n.slice(0, 4)
                    }(l, e.get(l))) t.add(i), null != r && null != g.A.getStreamForUser(i, r) && n.add(i)
            }
            return {
                shownUserIds: t,
                liveUserIds: n
