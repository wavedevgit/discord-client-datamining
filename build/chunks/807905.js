/** chunk id: 807905 params = (module,exports,require) **/
n.d(t, {
    A: () => R
}), n(321073);
var i = n(627968),
    s = n(64700),
    l = n(503698),
    a = n.n(l),
    r = n(311907),
    o = n(502572),
    d = n(732955),
    c = n(397927),
    u = n(775602),
    m = n(531260),
    g = n(657516),
    _ = n(983511),
    A = n(342744),
    x = n(87719),
    h = n(287809),
    p = n(166403),
    T = n(473145),
    f = n(927578),
    E = n(966275),
    S = n(311092),
    b = n(233413),
    C = n(652215),
    N = n(788868),
    v = n(985018),
    I = n(772882);

function j(e) {
    let {
        guildBoostSlot: t,
        isCancellable: n,
        onCancel: l,
        onUncancel: a,
        premiumSubscription: r,
        useReducedMotion: u,
        fractionalState: m
    } = e, g = s.useMemo(() => null != t.cooldownEndsAt ? new Date(t.cooldownEndsAt) : null, [t]), _ = (0, T.I5)(t), A = r?.isPaused === !0 && m === N.xc.NONE, x = v.intl.string(v.t.mOWsF1);
    return (0, i.jsxs)("li", {
        className: I.Hp,
        children: [(0, i.jsxs)("div", {
            className: I.YL,
            children: [(0, i.jsx)(b.A, {
                className: I.W6,
                hasCooldown: t.isOnCooldown(),
                isCanceled: _,
                useReducedMotion: u
            }), _ && null != r ? (0, i.jsx)(c.Text, {
                color: "text-muted",
                variant: "text-sm/medium",
                children: v.intl.format(v.t.Z4ULRD, {
                    date: r.currentPeriodEnd
                })
            }) : t.isOnCooldown() && null != g ? (0, i.jsx)(S.A, {
                cooldown: g.getTime()
            }) : (0, i.jsx)(c.Text, {
                className: I.__invalid_unappliedGuildBoostSlotDescription,
                color: "text-muted",
                variant: "text-sm/medium",
                children: v.intl.string(v.t["2mcafz"])
            })]
        }), (0, i.jsxs)("div", {
            className: I.E7,
            children: [n && !_ && (0, i.jsx)(o.A, {
                shouldShow: A,
                text: x,
                "aria-label": x.toString(),
                children: e => (0, i.jsx)("div", {
                    className: I.LB,
                    children: (0, i.jsx)(d.QWc, {
                        ...e,
                        variant: "primary",
                        text: v.intl.string(v.t.twFU3R),
                        onClick: () => l(t),
                        disabled: A,
                        size: "sm"
                    })
                })
            }), _ && (0, i.jsx)(o.A, {
                shouldShow: A,
                text: x,
                "aria-label": x.toString(),
                children: e => (0, i.jsx)("div", {
                    className: I.LB,
                    children: (0, i.jsx)(d.QWc, {
                        ...e,
                        variant: "primary",
                        text: v.intl.string(v.t["2glQNp"]),
                        onClick: () => a(t),
                        disabled: A,
                        size: "sm"
                    })
                })
            })]
        })]
    }, t.id)
}

function y(e) {
    (0, c.mMO)(async () => t => (0, i.jsx)(_.default, {
        ...t,
        guildBoostSlot: e
    }))
}

function O(e) {
    (0, c.mMO)(async () => t => (0, i.jsx)(A.default, {
        ...t,
        guildBoostSlotId: e.id
    }))
}
let R = function(e) {
    let t, n, l, {
            guildBoostSlots: _
        } = e,
        A = (0, r.bG)([p.A], () => p.A.getPremiumTypeSubscription()),
        S = A?.isPausedOrPausePending === !0,
        {
            fractionalState: b
        } = (0, m.A)(),
        R = (0, r.bG)([u.A], () => u.A.useReducedMotion),
        P = (0, r.bG)([h.default], () => h.default.getCurrentUser()),
        L = b === N.xc.FP_SUB_PAUSED,
        D = S && !L,
        {
            appliedGuildBoostSlots: G,
            unappliedGuildBoostSlots: M,
            numActiveGuildBoostSlots: k,
            hasCooldownBoosts: U,
            allGuildBoostsAreOnCooldown: w
        } = s.useMemo(() => {
            let e = [],
                t = [],
                n = 0,
                i = !1,
                s = !0;
            return _.forEach(l => {
                !(0, T.I5)(l) && n++, l.isOnCooldown() ? i = !0 : s = !1, null != l.premiumGuildSubscription ? e.push(l) : t.push(l)
            }), {
                appliedGuildBoostSlots: e,
                unappliedGuildBoostSlots: t,
                numActiveGuildBoostSlots: n,
                hasCooldownBoosts: i,
                allGuildBoostsAreOnCooldown: s
            }
        }, [_]),
        V = null != A ? f.Ay.getNumIncludedPremiumGuildSubscriptionSlots(A.planId) : 0,
        B = Math.max(0, V - G.length),
        F = k > V,
        H = V === _.length,
        z = H ? B : 1,
        Y = s.useMemo(() => {
            let e = [];
            for (let t = 0; t < z; t++) e.push((0, i.jsx)(E.A, {
                className: I.YA,
                useReducedMotion: R
            }, t));
            return e
        }, [z, R]),
        X = s.useMemo(() => M.find(e => e.isAvailable()), [M]);
    if (0 === M.length) return null;
    let K = M.length;
    if (t = H ? v.intl.formatToPlainString(w ? v.t["3DW6Dc"] : v.t["/u15Qc"], {
            numUnappliedGuildBoostSlots: K
        }) : v.intl.formatToPlainString(w ? v.t["3DW6Dc"] : v.t.BPadnO, {
            numUnappliedGuildBoostSlots: K
        }), f.Ay.isPremium(P)) {
        let e = (e, t) => (0, i.jsx)(c.DUT, {
            className: I.nw,
            tag: "span",
            onClick: () => {
                (0, x.e)()
            },
            children: e
        }, t);
        n = w ? v.intl.format(v.t.omcpSE, {
            learnMoreHook: e
        }) : v.intl.format(v.t["5mAkVi"], {
            numUnappliedGuildBoostSlots: K,
            learnMoreHook: e
        })
    } else n = w ? v.intl.string(v.t["8pcUZi"]) : v.intl.formatToPlainString(v.t.Kaw82o, {
        numUnappliedGuildBoostSlots: K
    });
    return l = S && b === N.xc.NONE ? v.intl.string(v.t.mOWsF1) : v.intl.string(v.t.xr4m5B), (0, i.jsx)("div", {
        className: I.iE,
        children: (0, i.jsxs)("div", {
            className: a()(I.Qs, [I.Yq]),
            children: [(0, i.jsxs)("div", {
                className: I.wx,
                children: [(0, i.jsxs)("div", {
                    className: I.RW,
                    children: [(0, i.jsx)("div", {
                        className: I.PS,
                        children: Y
                    }), (0, i.jsxs)("div", {
                        className: I.__invalid_headerCopy,
                        children: [(0, i.jsx)(c.Heading, {
                            className: I.gg,
                            variant: "heading-lg/bold",
                            children: t
                        }), (0, i.jsx)(c.Text, {
                            className: I.__invalid_headerSubheading,
                            color: "text-strong",
                            variant: "text-sm/normal",
                            children: n
                        })]
                    })]
                }), (0, i.jsx)("div", {
                    className: I.di,
                    children: (0, i.jsx)(o.A, {
                        shouldShow: null == X || D,
                        text: l,
                        "aria-label": l.toString(),
                        children: e => (0, i.jsx)(d.$nd, {
                            variant: "primary",
                            text: v.intl.string(v.t.BMx1iy),
                            ...e,
                            disabled: null == X || D,
                            onClick: null != X ? () => {
                                (0, c.mMO)(async () => e => (0, i.jsx)(g.default, {
                                    ...e,
                                    guildBoostSlots: [X],
                                    locationSection: C.JJy.SETTINGS_PREMIUM
                                }))
                            } : void 0
                        })
                    })
                })]
            }), (!H || U) && (0, i.jsx)("ul", {
                className: I.LU,
                children: M.map(e => (0, i.jsx)(j, {
                    guildBoostSlot: e,
                    isCancellable: F,
                    onCancel: y,
                    onUncancel: O,
                    premiumSubscription: A,
                    useReducedMotion: R,
                    fractionalState: b
                }, e.id))
            })]
        })
    })
}