...
function D(e) {
    ...
    let U = (0, d.bG)([u.A], () => u.A.getApplication(y.XR)),
        [P] = (0, d.bG)([A.A], () => null != D.id ? [A.A.getParticipants(D.id), A.A.getParticipantsVersion(D.id)] : [
            [], 0
        ], [D.id], h.hS),
        w = (0, d.yK)([E.A], () => {
            let e = new Set;
            for (let t of P)
                if ((0, R.Xw)(t) || (0, R.Ay)(t)) {
                    for (let n of E.A.getActivities(t.user.id))
                        if (n.application_id === U?.id || U?.linkedGames?.some(e => e.id === n.application_id) === !0) {
                            e.add(t.user.id);
                            break
                        }
        ...
        W = s.useMemo(() => {
            let e = new Set(k),
                ...
                s = t && n;
            return K.map(t => {
                let n = Object.entries(F[t.skuId] ?? {}).filter(t => {
                    let [n, i] = t;
    ...
    return s.useEffect(() => {
        0 !== H.length && I.default.track(j.HAw.COMMERCE_SHOP_VC_GIFT_BUTTON_VIEWED, {
            guild_id: D.guild_id,
            channel_id: D.id,
            sku_ids: H.map(e => e.id)
        })
    }, [D.id, D.guild_id, H]), (0, i.jsx)(m.A, {
