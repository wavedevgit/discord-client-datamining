...
var i = n(136722),
    ...
    _ = n(723176),
    m = n(453001);
let p = new r.A("GuildBasicChannels");
...
let E = new class {
    ...
    async getAsync(e) {
        let t = performance.now(),
            ...
            s = new Set(r);
        return this.synced = s, p.verbose(`loaded in ${a}ms (guilds: ${n.length}, synced: ${s.size} unsynced: ${l.length})`), {
            all: n,
            stale: l,
            channels: n.filter(e => {
                let [t, n] = e;
    ...
    handleChannelUpdates(e, t) {
        for (let n of e.channels.filter(e => null != e.guild_id)) g(d.A.getBasicChannel(n.id), n) && this.unsync(n.guild_id, t)
    }
    ...
    async handlePostConnectionOpen() {
        ...
        for (let a of (p.verbose(`scheduling basic_channel optimstic writes (guilds: ${i.length})`), n)) {
            if (null == this.synced || t !== _.A.database() || e !== l.A.lastTimeConnectedChanged()) break;
            if (!this.synced.has(a)) {
                p.verbose(`optimstically writing basic_channels (guild: ${a})`);
                try {
    ...
    onGuildUpdate(e, t, n, i) {
        (n.length > 0 || t.some(e => g(d.A.getBasicChannel(e.id), e))) && this.unsync(e, i)
    }
    onGuildSync(e, t) {
        this.unsync(e, t)
    }
