/** chunk id: 351405 params = (module,exports,require) **/
n.d(t, {
    P: () => N
}), n(321073);
var i = n(627968),
    l = n(64700),
    r = n(311907),
    s = n(397927),
    a = n(138175),
    o = n(99072),
    d = n(443795),
    c = n(961350),
    u = n(629016),
    m = n(375492),
    _ = n(290863),
    h = n(461213),
    p = n(287809),
    g = n(456060),
    A = n(850670),
    x = n(125017),
    f = n(104171),
    C = n(13403),
    E = n(536189),
    I = n(652215),
    v = n(272984),
    b = n(772234);

function T(e) {
    let {
        partyMembers: t,
        partySize: n,
        maxPartySize: l,
        guildId: r,
        activityActionType: a
    } = e, o = Math.max(n, t.length), d = (0, g.SJ)({
        maxPartySize: l,
        partySize: o,
        activityActionType: a
    }), c = [...t];
    for (; c.length < n && c.length < 8;) c.push(f.mt);
    for (; c.length < l && c.length < 8;) c.push(null);
    return (0, i.jsxs)("div", {
        className: b.UF,
        children: [c.length > 0 && (0, i.jsx)(f.Ay, {
            guildId: r,
            users: c,
            max: l > 0 ? Math.min(l, 8) : 8,
            size: f.DN.SIZE_16,
            dimEmptyUsers: !0
        }), (0, i.jsx)(s.Text, {
            variant: "text-xs/medium",
            color: "none",
            children: d
        })]
    })
}

function N(e) {
    let {
        analyticsLocations: t,
        app: n,
        channel: s,
        message: g,
        hideParty: b,
        onView: N
    } = e, S = (0, a.b)(n), y = (0, r.bG)([c.default], () => c.default.getId()), j = (0, r.bG)([_.A], () => {
        if (null == g.application) return _.A.findActivity(g.author.id, e => e.type === I.$pd.LISTENING);
        {
            let e = g.author.id;
            return (0, A.v)(g) && (e = e === y && s.isPrivate() ? s.getRecipientId() : y), _.A.getApplicationActivity(e, g.application.id)
        }
    }, [g, s, y]), L = (0, r.bG)([m.A, h.A], () => m.A.getApplicationActivity(S.id) ?? h.A.getApplicationActivity(S.id, !0), [S.id]), R = (0, r.yK)([u.A], () => null == j || null == j.party ? [] : Array.from(u.A.getParty(j.party.id) ?? []), [j]), {
        partySize: P,
        maxPartySize: D
    } = (0, x._)(j), M = l.useMemo(() => R.map(e => {
        let t = p.default.getUser(e);
        return null != t ? t : f.mt
    }), [R]), w = l.useMemo(() => (0, i.jsx)(T, {
        partyMembers: M,
        partySize: P,
        maxPartySize: D,
        guildId: s.guild_id,
        activityActionType: g.activity?.type
    }), [M, P, D, s.guild_id, g.activity?.type]);
    return (0, v.pH)(j?.party?.id) || S.id === d.HT.id ? (0, i.jsx)(E.A, {
        application: S,
        currentUserPresenceActivity: L,
        hideParty: b,
        message: g,
        onView: N,
        partyStatusElement: w,
        presenceActivity: j,
        guildId: s.guild_id
    }) : g.activity?.type === I.xL.STREAM_REQUEST ? (0, i.jsx)(o.A, {
        analyticsLocations: t,
        application: S,
        channel: s,
        currentUserId: y,
        message: g
    }) : (0, i.jsx)(C.A, {
        analyticsLocations: t,
        application: S,
        channel: s,
        currentUserId: y,
        currentUserPresenceActivity: L,
        hideParty: b,
        message: g,
        onView: N,
        partyStatusElement: w,
        presenceActivity: j
    })
}