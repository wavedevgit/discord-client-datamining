...
function $(e) {
    ...
    let tU = l.useCallback(e => {
        null != eq.current && (tt.info(`[QV] | seekTimeline | timeSec: ${e}`), eV(e / (eq.current.duration ?? 1) * 100), tN(), eA(!0), e7(!1), eq.current.currentTime = e, e$(W.id, e, eq.current.duration))
    }, [eq, e$, W.id, tN, tt]);
    l.useEffect(() => {
        let e = eq.current;
        if (null == e) return;
