...
function f(e) {
    let {
        ...
        transitionState: y,
        ...I
    } = e, [v, x] = i.useState(f ?? 0), [M, R] = i.useState(!1), [O, N] = (0, c.zhh)(() => ({
        scale: _.A.useReducedMotion ? 1 : .9,
        x: 0,
        y: 0,
        config: {
            friction: 30,
    ...
    i.useEffect(() => {
        if (null != t) return u._.subscribe(S.jej.MEDIA_MODAL_CLOSE, t), () => {
    ...
    i.useEffect(() => {
        if (y !== j.current) switch (j.current = y, y) {
            case c.ip4.ENTERING:
                ...
                break;
            case c.ip4.EXITING:
                _.A.useReducedMotion || (C(O.x, 0), C(O.y, 0), C(O.scale, .9))
        }
    }, [y, O]), y === c.ip4.HIDDEN && M && (R(!1), O.x.set(0), O.y.set(0), O.scale.set(1)), i.useEffect(() => {
        y === c.ip4.ENTERED && (d.A.disable(), d.A.enableTemp(s.w)), y === c.ip4.HIDDEN && (d.A.disable(), d.A.enableTemp(s.b));
        let e = () => {
