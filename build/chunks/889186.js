...
function S(e) {
    ...
    let [o, h] = s.useState(""), [f, g] = s.useState(() => new Set), {
        notifyFavoriteAdded: p
    } = (0, v.CJ)(), {
        hasHigherPrivileges: b
    } = (0, v.TW)("AddFavoriteChannelModal"), S = (0, i.bG)([A.A], () => A.A.getFavoriteChannels()), R = s.useMemo(() => new Set(Object.keys(S)), [S]), w = s.useCallback((e, t) => {
        if (e.type === c.rD.USER) {
            let l = m.A.getDMChannelFromUserId(e.record.id);
            return (!!t || null != l) && (null == l || !R.has(l.id))
        }
        return e.type === c.rD.GROUP_DM ? !R.has(e.record.id) : _.A.can(k.xBc.VIEW_CHANNEL, e.record) && (0, j.IF)(e.record, b) && !R.has(e.record.id)
    }, [R, b]), {
        results: T,
        updateSearchText: G
    } = (0, x.R)({
        includeMissingDMs: !0,
        channelFilter: w
    }), P = s.useMemo(() => [...f].filter(e => !R.has(e)).length, [R, f]), H = P >= 20, q = s.useMemo(() => P >= 2 ? L.intl.formatToPlainString(D.default.LbCa8x, {
        count: P
    }) : L.intl.string(D.default.xKXcSu), [P]), W = s.useCallback(e => {
        h(e), G(e)
    }, [G]), U = s.useCallback(e => {
        R.has(e) || g(t => {
            let l = 0;
...
function E(e) {
    let {
        ...
        disabled: s,
        onToggleChannel: r
    } = e, c = (0, i.bG)([p.A], () => p.A.getGuild(t.guild_id), [t.guild_id]), g = (0, d.Ay)(t), x = (0, i.bG)([b.default], () => t.isDM() ? b.default.getUser(t.recipients?.[0]) : null, [t]);
    return (0, n.jsx)(o.A, {
        listItemId: t.id,
