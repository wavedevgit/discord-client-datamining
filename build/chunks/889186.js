...
function j(e) {
    let {
    ...
    } = (0, b.CJ)(), x = (0, i.bG)([C.A], () => C.A.getFavoriteChannels()), j = s.useMemo(() => new Set(Object.keys(x)), [x]), E = s.useCallback((e, t) => {
        if (e.type === c.rD.USER) {
            let l = A.A.getDMChannelFromUserId(e.record.id);
            return (!!t || null != l) && (null == l || !j.has(l.id))
        }
        return e.type === c.rD.GROUP_DM ? !j.has(e.record.id) : m.A.can(_.xBc.VIEW_CHANNEL, e.record) && !j.has(e.record.id)
    }, [j]), {
        results: N,
        updateSearchText: G
    } = (0, g.R)({
        includeMissingDMs: !0,
        channelFilter: E
    }), R = s.useMemo(() => [...h].filter(e => !j.has(e)).length, [j, h]), w = R >= 20, I = s.useMemo(() => R >= 2 ? M.intl.formatToPlainString(L.default.LbCa8x, {
        count: R
    }) : M.intl.string(L.default.xKXcSu), [R]), T = s.useCallback(e => {
        o(e), G(e)
    }, [G]), H = s.useCallback(e => {
        j.has(e) || f(t => {
            let l = 0;
...
function v(e) {
    let {
        ...
        disabled: s,
        onToggleChannel: r
    } = e, c = (0, i.bG)([p.A], () => p.A.getGuild(t.guild_id), [t.guild_id]), f = (0, d.Ay)(t), g = (0, i.bG)([x.default], () => t.isDM() ? x.default.getUser(t.recipients?.[0]) : null, [t]);
